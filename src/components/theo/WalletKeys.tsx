  ...
  return (
    <div className="bg-card border border-border rounded-xl shadow-xs overflow-hidden">
      <div className="flex items-center gap-2.5 px-5 py-3.5 border-b border-border" style={{ background: "hsl(var(--theo-blue-soft))" }}>
        ...
        <div className="font-bold" style={{ fontSize: 13, color: "hsl(var(--theo-blue))" }}>{t("wallet.credentials.title")}</div>
      </div>
      ...
      <div className="px-5 py-4">
        ...
        >
          ...
          <div style={{ fontSize: 12, color: "hsl(var(--theo-blue))", fontWeight: 600 }}>
            {t("wallet.credentials.warning")}
          </div>
        </div>

        {loading ? (
          <div style={{ fontSize: 13, color: "hsl(var(--theo-mid))" }}>{t("wallet.credentials.loading")}</div>
        ) : wallets.length === 0 ? (
          <div style={{ fontSize: 13, color: "hsl(var(--theo-mid))" }}>{t("wallet.credentials.empty")}</div>
        ) : (
              ...
              <div key={w.id} className="border border-border rounded-lg" style={{ padding: 14 }}>
                    ...
                    <div className="flex items-center gap-2" style={{ flex: 1 }}>
                      ...
                      {savedId === w.id && (
                        <span className="flex items-center gap-1" style={{ fontSize: 11, fontWeight: 600, color: "hsl(var(--theo-cyan))" }}>
                          <Check style={{ width: 11, height: 11 }} /> {t("wallet.credentials.saved")}
                        </span>
                      )}
                    </div>
                  )}
                ...
                <div className="mb-2">
                  <div className="font-bold uppercase mb-1" style={{ fontSize: 10, letterSpacing: "0.12em", color: "hsl(var(--theo-mid))" }}>
                    {t("wallet.credentials.publicKey")}
                  </div>
                  <div className="flex items-center gap-2">
                    ...
                    >
                      <Copy style={{ width: 11, height: 11 }} /> {t("wallet.credentials.copy")}
                    </button>
                  </div>
                </div>
                ...
                <div>
                  ...
                  {!w.has_signing_key ? (
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      >
                        {revealed[w.id] ? (
                          <><EyeOff style={{ width: 11, height: 11 }} /> {t("wallet.credentials.hide")}</>
                        ) : (
                          <><Eye style={{ width: 11, height: 11 }} /> {revealing[w.id] ? t("wallet.credentials.loading2") : t("wallet.credentials.reveal")}</>
                        )}
                      </button>
                      {revealed[w.id] && (
                        <button
