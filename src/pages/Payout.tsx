...
import { useLocale, NUMBER_LOCALE } from "@/lib/locale";
import { resolveEffectiveCustomerId } from "@/lib/customer";
import { INTERNAL_PAYOUT_MEMOS } from "@/lib/search";
...
type SavedRecipient = {
  ...
  label: string | null;
  memo: string | null;
  memo_type: "text" | "id" | null;
};

const STATUS_STYLE: Record<string, { bg: string; color: string }> = {
  COMPLETED: { bg: "#EFFBF3",                    color: "#1A7F37" },
  PENDING:   { bg: "hsl(var(--theo-gold-soft))", color: "#7A5F00" },
...
export default function Payout() {
  ...
  // Redirect users who lack payout_send permission.
  // Must wait for BOTH useRoles AND usePermissions to finish — they are
  // independent async queries; whichever finishes first must not trigger a
  // redirect while the other is still pending.
  const authLoading = rolesLoading || permsLoading;
  const canSendPayout = isOwner || isAdmin || can("payout_send");
  const canViewBalances = isOwner || isAdmin || can("view_balances");
  useEffect(() => {
    if (!authLoading && !canSendPayout) {
      navigate("/dashboard", { replace: true });
    }
  }, [authLoading, canSendPayout, navigate]);
  ...
  const { highlightId, refs: payoutHighlightRefs } = useSearchHighlight<HTMLDivElement>(!payoutsLoading);
  ...
  const filteredPayouts = query.trim()
    ? payouts.filter((p) => {
        ...
        const memoOk = p.memo && !(INTERNAL_PAYOUT_MEMOS as readonly string[]).includes(p.memo);
        return (
          p.recipient_name.toLowerCase().includes(q) ||
          String(p.amount_usdc).includes(q) ||
          (p.stellar_tx_hash ?? "").toLowerCase().includes(q) ||
          (memoOk && (p.memo ?? "").toLowerCase().includes(q))
        );
      })
    : payouts;
  ...
  const loadPayouts = async (cid: string) => {
    ...
    const { data } = await supabase
      ...
      .eq("customer_id", cid)
      // NULL != 'x' is NULL in SQL (not TRUE), so neq silently drops memo-less rows.
      // Use or() to keep payouts with no memo alongside those with a real memo value.
      .or("memo.is.null,and(memo.neq.internal-transfer,memo.neq.blend-withdraw)")
  ...
  const loadSavedRecipients = async (cid: string) => {
    const { data } = await supabase
      ...
      .select("id, name, stellar_address, label, memo, memo_type")
      .eq("customer_id", cid)
  ...
  const handleSend = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!sourceWalletId) { toast.error(t("payout.selectSource")); return; }
    ...
    if (!parsedAmount || parsedAmount <= 0) { toast.error(t("payout.enterValidAmount")); return; }
    ...
    if (parsedAmount > availableBalance + 0.01) {
      toast.error(
          t("payout.insufficient").replace("{amount}", availableBalance.toLocaleString(NUMBER_LOCALE, { minimumFractionDigits: 2, maximumFractionDigits: 2 }))
      );
      return;
    }

    // Validate memo by type — never silently truncate/coerce (wrong memo can
    // misroute funds to an exchange irreversibly).
    const memoVal = memo.trim();
    if (memoVal) {
      if (memoType === "text" && new TextEncoder().encode(memoVal).length > 28) {
        toast.error(t("payout.memo.errTextLong"));
        return;
      }
      if (memoType === "id") {
        if (!/^\d+$/.test(memoVal)) { toast.error(t("payout.memo.errIdNumeric")); return; }
        if (BigInt(memoVal) > 18446744073709551615n) { toast.error(t("payout.memo.errIdRange")); return; }
      }
    }
    ...
    try {
      const res = await supabase.functions.invoke("send-payment", {
        body: { sourceWalletId, recipientAddress, recipientName, amount: parsedAmount, memo, memoType },
      });

      if (res.data && (res.data as { ok?: boolean }).ok === false) {
        toast.error((res.data as { error?: string }).error ?? t("payout.sendFailed"));
      ...
      if (saveAfterSend && !isAlreadySaved && customerId) {
        const saveMemoVal = memo.trim();
        await supabase.from("saved_recipients").upsert({
          customer_id: customerId,
          name: recipientName.trim(),
          stellar_address: recipientAddress.trim(),
          memo: saveMemoVal || null,
          memo_type: saveMemoVal ? memoType : null,
        }, { onConflict: "customer_id,stellar_address" });
        await loadSavedRecipients(customerId);
      }
  ...
  const memoBytes = memoType === "text" ? new TextEncoder().encode(memo.trim()).length : 0;
    ...
    <AppLayout>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "3fr 2fr" }}>
        ...
        <div className="bg-card border border-border rounded-xl shadow-xs self-start" style={{ padding: "16px 20px 14px" }}>
          <div className="font-bold mb-1" style={{ fontSize: 13, color: "hsl(var(--theo-blue))" }}>{t("payout.new")}</div>

          <div className="flex border-b border-border mb-2 mt-2" style={{ overflowX: "auto" }}>
            <button style={tabStyle("single")} onClick={() => setTab("single")}>{t("payout.tab.single")}</button>
            <button style={tabStyle("bulk")} onClick={() => setTab("bulk")}>{t("payout.tab.bulk")}</button>
            <button style={tabStyle("global")} onClick={() => setTab("global")}>{t("payout.tab.global")}</button>
          </div>
          ...
          {tab === "single" ? (
            <form onSubmit={handleSend}>
              ...
              <div style={{ marginBottom: 10 }}>
                <div className="flex items-center justify-between" style={{ marginBottom: 8 }}>
                  <label style={labelStyle}>{t("payout.savedRecipients")}</label>
                  {savedRecipients.length > 0 && (
                    ...
                    >
                      {t("payout.manage")} <ChevronDown style={{ width: 11, height: 11, transform: "rotate(-90deg)" }} />
                    </button>
                  )}
                </div>
                      ...
                      <div style={{
                        ...
                        {savedRecipients.length > 4 && (
                          <div style={{ padding: "8px 10px", borderBottom: "1px solid hsl(var(--theo-light))" }}>
                            <input
                              autoFocus
                              placeholder={t("payout.searchRecipients")}
                        ...
                        {filteredSaved.length === 0 ? (
                          <div style={{ padding: "10px 12px", fontSize: 13, color: "hsl(var(--theo-mid))" }}>{t("common.noResults")}</div>
                        ) : (
                          filteredSaved.map((r) => (
                            <button
                              key={r.id}
              ...
              <div className="grid gap-3 mb-2.5" style={{ gridTemplateColumns: "1fr 1fr" }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", minHeight: 28, marginBottom: 6 }}>
                    <label style={{ ...labelStyle, marginBottom: 0 }}>{t("payout.recipientName")}</label>
                  </div>
                  <input
                ...
                <div>
                  <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", minHeight: 28, marginBottom: 6 }}>
                    <label style={{ ...labelStyle, marginBottom: 0 }}>{t("payout.destination")}</label>
                      ...
                      {chainDropdownOpen && (
                        <div style={{
                          ...
                          <div style={{ padding: "7px 12px", fontSize: 10, color: "hsl(var(--theo-mid))", borderTop: "1px solid hsl(var(--theo-light))", lineHeight: 1.5 }}>
                            {t("payout.cctpSoon")} <a href="https://www.circle.com/cross-chain-transfer-protocol" target="_blank" rel="noopener noreferrer" style={{ fontWeight: 700, color: "hsl(var(--theo-blue))", textDecoration: "none" }}>CCTP</a>
                          </div>
                        </div>
                      )}
                    </div>
                  ...
                  {selectedChain.status === "live" && addrState === "valid" && (() => {
                    ...
                    if (trustStatus === "no_trust") return (
                      <div style={{ marginTop: 6, padding: "8px 10px", borderRadius: 7, background: "#FFFBEB", border: "1px solid #FDE68A", display: "flex", alignItems: "flex-start", gap: 7 }}>
                        ...
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 700, color: "#92400E" }}>{t("payout.usdcNotEnabled")}</div>
                    ...
                    if (trustStatus === "not_authorized") return (
                      <div style={{ marginTop: 6, padding: "8px 10px", borderRadius: 7, background: "#EFF6FF", border: "1px solid #BFDBFE", display: "flex", alignItems: "flex-start", gap: 7 }}>
                        ...
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 700, color: "#1E3A8A" }}>{t("payout.trustPending")}</div>
                    ...
                    if (trustStatus === "not_found") return (
                      <div style={{ marginTop: 6, padding: "8px 10px", borderRadius: 7, background: "#FEE2E2", border: "1px solid #FECACA", display: "flex", alignItems: "flex-start", gap: 7 }}>
                        ...
                        <div>
                          <div style={{ fontSize: 11, fontWeight: 700, color: "#7F1D1D" }}>{t("payout.accountNotFound")}</div>
              ...
              <div style={{ marginBottom: 10 }}>
                <div className="flex items-center justify-between" style={{ marginBottom: 4 }}>
                  <label style={{ ...labelStyle, marginBottom: 0 }}>{t("payout.amount")}</label>
                  {(() => {
                    const sw = wallets.find((w) => w.id === sourceWalletId);
                    return sw && canViewBalances ? (
                      <button
                        type="button"
              ...
              <div style={{ marginBottom: 10 }}>
                <div className="flex items-center justify-between" style={{ marginBottom: 4 }}>
                  <label style={{ ...labelStyle, marginBottom: 0 }}>{t("payout.sourceAccount")}</label>
                  {(() => {
                    ...
                    return (
                      <span style={{
                        ...
                        {(Math.floor(bal * 100) / 100).toLocaleString(NUMBER_LOCALE, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USDC {t("payout.available")}
                      </span>
                    );
                  })()}
                </div>
                {walletsLoading ? (
                  ...
                  <div style={{ fontSize: 13, color: "hsl(var(--theo-mid))" }}>{t("payout.noAccounts")}</div>
                ) : (
                  <select
              ...
              <div style={{ marginBottom: 10 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 4 }}>
                  <label style={{ ...labelStyle, marginBottom: 0 }}>{t("payout.memo")}</label>
                  <select
                    value={memoType}
                    onChange={(e) => { setMemoType(e.target.value as "text" | "id"); setMemo(""); }}
                    style={{ fontSize: 11, fontWeight: 600, color: "hsl(var(--theo-mid))", border: "1px solid hsl(var(--theo-light))", borderRadius: 6, padding: "2px 6px", background: "#fff", fontFamily: "inherit", cursor: "pointer" }}
                  >
                    <option value="text">{t("payout.memo.typeText")}</option>
                    <option value="id">{t("payout.memo.typeId")}</option>
                  </select>
                </div>
                <div style={{ fontSize: 11, color: "hsl(var(--theo-mid))", lineHeight: 1.4, marginBottom: 6 }}>
                  {t("payout.memo.typeGuide")}
                </div>
                <input
                  style={{ ...inputStyle, marginBottom: 0 }}
                  type="text"
                  inputMode={memoType === "id" ? "numeric" : "text"}
                  placeholder={memoType === "id" ? "e.g. 8847291" : "e.g. April salary — supplier payment"}
                  value={memo}
                  onChange={(e) => setMemo(memoType === "id" ? e.target.value.replace(/\D/g, "").slice(0, 20) : e.target.value)}
                  maxLength={memoType === "id" ? 20 : undefined}
                />
                {memoType === "text" && memo.trim() && (
                  <div style={{ fontSize: 11, color: memoBytes > 28 ? "#DC2626" : "hsl(var(--theo-mid))", marginTop: 4, textAlign: "right" }}>
                    {memoBytes} / 28 bytes
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center justify-between mt-1">
                ...
                <label style={{ display: "flex", alignItems: "center", gap: 7, cursor: "pointer", userSelect: "none" }}>
                  {isAlreadySaved ? (
                    ...
                    <>
                      ...
                      />
                      <span style={{ fontSize: 12, color: "hsl(var(--theo-mid))", fontWeight: 500 }}>{t("payout.saveRecipient")}</span>
                    </>
                  )}
                </label>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    type="submit"
                    disabled={sending || wallets.length === 0 || !canSendPayout || trustStatus === "not_found" || trustStatus === "checking"}
                    className="flex items-center gap-1.5 font-bold text-white"
                    style={{ background: "hsl(var(--theo-blue))", borderRadius: 8, padding: "8px 16px", fontSize: 13, border: "none", cursor: (sending || !canSendPayout || trustStatus === "not_found" || trustStatus === "checking") ? "not-allowed" : "pointer", fontFamily: "inherit", opacity: (sending || !canSendPayout || trustStatus === "not_found" || trustStatus === "checking") ? 0.5 : 1 }}
                  >
                    {sending ? <><Loader2 style={{ width: 13, height: 13, animation: "spin 1s linear infinite" }} /> {t("payout.submitting")}</> : t("payout.submit")}
                  </button>
                </div>
              </div>
              {!canSendPayout && (
            ...
            <>
              ...
              >
                ...
                <div style={{ fontSize: 13, fontWeight: 600, color: "hsl(var(--theo-blue))" }}>{t("payout.uploadCsv")}</div>
                <div style={{ fontSize: 11, color: "hsl(var(--theo-mid))", marginTop: 4 }}>
                  {t("payout.bulk.columns")} name, account_id, amount_usdc, note
                </div>
              </div>
              <div style={{ marginTop: 12 }}>
                <a href="#" style={{ fontSize: 12, color: "hsl(var(--theo-cyan))", fontWeight: 600, textDecoration: "none" }}>
                  {t("payout.bulk.downloadTemplate")}
                </a>
              </div>
            </>
          ) : (() => {
            ...
            const allFilled =
              ...
              bankAmountRaw > 0 &&
              !!sourceWalletId;
            const disabled = bankBusy || !allFilled || overBalance || !canSendPayout;

            const fmt = (n: number) => n.toLocaleString(NUMBER_LOCALE, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
              ...
              <>
                <div className="rounded-xl mb-4 flex items-start gap-2.5" style={{ background: "hsl(var(--theo-blue-soft))", border: "1px solid hsl(var(--theo-cyan))", padding: "12px 14px" }}>
                  ...
                  <div style={{ fontSize: 12, color: "hsl(var(--theo-blue))", lineHeight: 1.6 }}>
                    {t("payout.bank.notice.before")}{" "}<strong>OwlPay</strong>{" "}{t("payout.bank.notice.after")}
                  </div>
                </div>
                ...
                <div style={{ marginBottom: 12 }}>
                  <div className="flex items-center justify-between" style={{ marginBottom: 6 }}>
                    <label style={{ ...labelStyle, marginBottom: 0 }}>{t("payout.sourceAccount")} <span style={{ color: "#C00" }}>*</span></label>
                    {selectedBankWallet && (
                      <span
                        style={{
                      ...
                      >
                        {t("payout.available")}: {canViewBalances ? `${bankWalletBalance.toLocaleString(NUMBER_LOCALE, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USDC` : "—"}
                      </span>
                    )}
                  </div>
                  {walletsLoading ? (
                ...
                <div style={{ marginBottom: 12 }}>
                  <label style={labelStyle}>{t("payout.recipientName")} <span style={{ color: "#C00" }}>*</span></label>
                  <input style={inputStyle} value={bankRecipientName} onChange={(e) => setBankRecipientName(e.target.value)} placeholder="Jane Doe" />
                </div>

                <div style={{ marginBottom: 12 }}>
                  <label style={labelStyle}>{t("payout.bank.bankName")} <span style={{ color: "#C00" }}>*</span></label>
                  <input style={inputStyle} value={bankBankName} onChange={(e) => setBankBankName(e.target.value)} placeholder="HSBC, BBVA, etc." />
                </div>

                <div style={{ marginBottom: 12 }}>
                  <label style={labelStyle}>{t("payout.bank.accountNumber")} <span style={{ color: "#C00" }}>*</span></label>
                ...
                <div style={{ marginBottom: 12 }}>
                  <label style={labelStyle}>{t("payout.bank.routingCode")} <span style={{ color: "#C00" }}>*</span></label>
                  <input style={inputStyle} value={bankRoutingCode} onChange={(e) => setBankRoutingCode(e.target.value)} placeholder="NWBKGB2L" />
                </div>

                <div style={{ marginBottom: 12 }}>
                  <label style={labelStyle}>{t("payout.amount")} <span style={{ color: "#C00" }}>*</span></label>
                  <input
                    style={inputStyle}
                    inputMode="decimal"
                  ...
                  {overBalance && (
                    <div style={{ fontSize: 11, color: "hsl(var(--destructive))", marginTop: 6, fontWeight: 600 }}>
                      {t("payout.bank.insufficient").replace("{amount}", fmt(totalDebit))}
                    </div>
                  )}
                  {bankAmountRaw > 50000 && (
                    <div style={{ fontSize: 11, color: "hsl(var(--theo-cyan))", marginTop: 6, fontWeight: 600 }}>
                      {t("payout.bank.volumeIncentive")}
                    </div>
                  )}
                </div>
                ...
                <div className="rounded-xl mb-4 p-4" style={{ background: "hsl(var(--theo-cream))", border: "1px solid hsl(var(--theo-light))" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "hsl(var(--theo-cyan))", marginBottom: 10 }}>
                    {t("payout.bank.transferSummary")}
                  </div>
                  <div className="flex justify-between" style={{ fontSize: 12, marginBottom: 6 }}>
                    <span style={{ color: "hsl(var(--theo-mid))" }}>{t("payout.bank.principal")}</span>
                    <span style={{ fontWeight: 700, color: "hsl(var(--theo-blue))" }}>${fmt(bankAmountRaw)} USDC</span>
                  </div>
                  <div className="flex justify-between" style={{ fontSize: 12, marginBottom: 6 }}>
                    <span style={{ color: "hsl(var(--theo-mid))" }}>{t("payout.bank.wireFee")} <span style={{ opacity: 0.7 }}>({t("payout.bank.flat")})</span></span>
                    <span style={{ fontWeight: 700, color: "hsl(var(--theo-blue))" }}>${fmt(WIRE_FLAT_FEE)} USDC</span>
                  </div>
                  <div className="flex justify-between" style={{ fontSize: 12, marginBottom: 6 }}>
                    <span style={{ color: "hsl(var(--theo-mid))" }}>{t("payout.bank.processingFee")} <span style={{ opacity: 0.7 }}>({(variableBps / 100).toFixed(2)}%)</span></span>
                  ...
                  <div className="flex justify-between" style={{ fontSize: 13, marginTop: 8, paddingTop: 8, borderTop: "1px solid hsl(var(--theo-light))" }}>
                    <span style={{ fontWeight: 700, color: "hsl(var(--theo-blue))" }}>{t("payout.total")}</span>
                  ...
                  <div className="flex justify-between items-center" style={{ fontSize: 13, marginTop: 10, paddingTop: 10, borderTop: "1px dashed hsl(var(--theo-cyan))" }}>
                    <span style={{ fontWeight: 800, color: "hsl(var(--theo-blue))" }}>{t("payout.netDelivery")}</span>
                    <span style={{ fontWeight: 800, color: "hsl(var(--theo-blue))" }}>
                      {t("payout.bank.recipientReceives.before")} <span style={{ color: "hsl(var(--theo-cyan))" }}>${fmt(netDelivered)}</span> {t("payout.bank.recipientReceives.after")}
                    </span>
                  </div>
                </div>
                ...
                >
                  {bankBusy ? t("payout.bank.initiating") : t("payout.bank.confirm")}
                </button>
              </>
            );
          })()}
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-xs">
          <div className="flex items-center justify-between mb-4">
            <div className="font-bold" style={{ fontSize: 13, color: "hsl(var(--theo-blue))" }}>{t("payout.recent")}</div>
            {query.trim() && !payoutsLoading && (
              <span style={{ fontSize: 11, color: "hsl(var(--theo-mid))" }}>
                {filteredPayouts.length} {t("payout.resultsOf")} {payouts.length}
              </span>
            )}
          </div>
          {payoutsLoading ? (
            ...
            <div style={{ fontSize: 13, color: "hsl(var(--theo-mid))" }}>{t("payout.empty")}</div>
          ) : filteredPayouts.length === 0 ? (
            <div style={{ fontSize: 13, color: "hsl(var(--theo-mid))" }}>{t("payout.noResults")} "{query}"</div>
          ) : (
            <div className="flex flex-col gap-2.5">
              {filteredPayouts.map((p, i) => {
                const s = STATUS_STYLE[p.status] ?? STATUS_STYLE.PENDING;
                const date = new Date(p.created_at).toLocaleDateString(locale, { month: "short", day: "numeric", year: "numeric" });
                const isSaved = savedRecipients.some((r) => r.name === p.recipient_name);
                const isHighlighted = highlightId === p.id;
                return (
                  <div
                    key={p.id}
                    ref={(el) => { payoutHighlightRefs.current[p.id] = el; }}
                    className="flex justify-between items-start py-2.5"
                    style={{
                      borderBottom: i < filteredPayouts.length - 1 ? "1px solid hsl(var(--theo-light))" : "none",
                      background: isHighlighted ? "hsl(var(--theo-blue-soft))" : undefined,
                      borderRadius: isHighlighted ? 6 : undefined,
                      padding: isHighlighted ? "10px 8px" : undefined,
                  ...
                  >
                    <div>
                      ...
                      {p.stellar_tx_hash && canViewBalances && (
                        <a
                    ...
                    <div className="text-right">
                      <div style={{ fontWeight: 700, fontSize: 14, color: "hsl(var(--theo-blue))" }}>
                        {canViewBalances ? `$${fmtUSDC(Number(p.amount_usdc))}` : "—"}
                      </div>
                      <span className="rounded-full font-bold" style={{ fontSize: 11, background: s.bg, color: s.color, padding: "2px 8px" }}>
                        {p.status === "COMPLETED" ? t("payout.status.paid") : p.status === "FAILED" ? t("common.status.failed") : t("common.status.processing")}
                      </span>
                    </div>
                  </div>
                );
        ...
        >
          ...
          >
            <div style={{ padding: "16px 20px", borderBottom: "1px solid hsl(var(--theo-light))", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "hsl(var(--theo-cyan))" }}>{t("payout.savedRecipients")}</div>
                <div style={{ fontSize: 16, fontWeight: 700, color: "hsl(var(--theo-ink))", marginTop: 2 }}>{t("payout.manage")}</div>
              </div>
              <button
                type="button"
                onClick={() => { setShowManage(false); setEditingId(null); }}
      ...
      {confirmDelete && (
        ...
        >
          ...
          >
            <div style={{ fontSize: 15, fontWeight: 700, color: "hsl(var(--theo-ink))", marginBottom: 8 }}>{t("payout.removeRecipient")}</div>
