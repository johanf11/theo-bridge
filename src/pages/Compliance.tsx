...
import { NUMBER_LOCALE, capitalizeDate, useLocale } from "@/lib/locale";
...
export default function Compliance() {
  ...
  const fmtTs = (d: Date) =>
    ...
    " · " +
    capitalizeDate(d.toLocaleDateString(locale, { day: "2-digit", month: "short", year: "numeric" }));
    ...
    <AppLayout>
      <div style={{ maxWidth: 960, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 24 }}>
          <StatusPill label={t("compliance.kybActive")} />
          <StatusPill label={t("compliance.reservesVerified")} />
          <StatusPill label={t("compliance.stellarTestnet")} variant="blue" />
        </div>

        {/* Error banner */}
        {state === "error" && (
          <div style={{ padding: "12px 16px", borderRadius: 10, background: R_BG, border: `1px solid ${R_BD}`, color: R_FG, fontSize: 13, marginBottom: 18 }}>
            <span style={{ fontWeight: 700 }}>{t("compliance.networkError")} </span>{error}
          </div>
        )}
        ...
        <Panel>
          <PanelHead
            title={t("compliance.proofOfReserve")}
            meta={fetchedAt ? `${t("compliance.asOf")} ${capitalizeDate(fetchedAt.toLocaleDateString(locale, { day: "2-digit", month: "short", year: "numeric" }))} · ${fetchedAt.toLocaleTimeString(locale, { hour: "2-digit", minute: "2-digit" })}` : t("compliance.na")}
          />
          <div style={{ padding: "20px 22px" }}>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 0 }}>
              ...
              <div style={{ paddingRight: 24 }}>
                ...
                <div style={{ fontSize: 12, color: MID, marginTop: 8, lineHeight: 1.45 }}>
                  {t("compliance.htgcInCirculationDesc")}
                </div>
              ...
              <div style={{ paddingLeft: 24, borderLeft: `1px solid ${LT}` }}>
                <div style={{ fontSize: 10, fontWeight: 700, letterSpacing: "0.14em", textTransform: "uppercase", color: MID, marginBottom: 10 }}>
                  {t("compliance.bankReserve")}
                </div>
                <div style={{ fontSize: 28, fontWeight: 800, color: N, letterSpacing: "-0.02em", fontVariantNumeric: "tabular-nums", lineHeight: 1.1 }}>
                  {displayedHtgBalance != null ? fmtN(displayedHtgBalance) : t("compliance.na")}
                  <span style={{ fontSize: 12, fontWeight: 700, color: MID, letterSpacing: "0.06em", marginLeft: 6 }}>HTG</span>
                </div>
                <div style={{ fontSize: 12, color: MID, marginTop: 8, lineHeight: 1.45 }}>
                  {t("compliance.bankReserveDesc")}
                </div>
                <div style={{ fontSize: 11, color: MID, marginTop: 8 }}>
                  {attestation
                    ? <>{t("compliance.attested")} {attestation.period_label} {t("compliance.by")} <span style={{ fontStyle: "italic" }}>{t("compliance.attestationPartnerTbd")}</span></>
                    : t("compliance.awaitingAttestation")}
                </div>
              </div>
            </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                ...
                <div>
                  <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.10em", textTransform: "uppercase", color: ratioFg }}>
                    {t("compliance.reserveRatio")} · {ratioState === "ok" ? t("compliance.ratioFullyCollateralized") : ratioState === "warn" ? t("compliance.ratioApproachingThreshold") : t("compliance.ratioBelowThreshold")}
                  </div>
                  <div style={{ fontSize: 11.5, color: MID, marginTop: 2 }}>
                    {t("compliance.reserveRatioDesc")}
                  </div>
                </div>
              </div>
              <div style={{ fontSize: 26, fontWeight: 800, color: ratioFg, fontVariantNumeric: "tabular-nums", letterSpacing: "-0.02em", flexShrink: 0 }}>
                {ratio != null ? `${ratio.toFixed(2)}` : t("compliance.na")}
            ...
            <div style={{
              ...
              <a href={`https://stellar.expert/explorer/testnet/asset/HTGC-${HTGC_ISSUER}`} target="_blank" rel="noreferrer"
                ...
                <ExternalLink style={{ width: 11, height: 11 }} /> {t("compliance.verifyIssuer")} →
              </a>
              ...
              <a href={`https://stellar.expert/explorer/testnet/account/${HTGC_DISTRIBUTOR}`} target="_blank" rel="noreferrer"
                style={{ color: N, display: "flex", alignItems: "center", gap: 5 }}>
                <ExternalLink style={{ width: 11, height: 11 }} /> {t("compliance.verifyDistributor")} →
              </a>
              <span style={{ color: LT }}>·</span>
              <a href={`https://stellar.expert/explorer/testnet/account/${THEO_TREASURY}`} target="_blank" rel="noreferrer"
                style={{ color: N, display: "flex", alignItems: "center", gap: 5 }}>
                <ExternalLink style={{ width: 11, height: 11 }} /> {t("compliance.verifyTreasury")} →
              </a>
              <>
                <span style={{ color: LT }}>·</span>
                <span style={{ color: MID, display: "flex", alignItems: "center", gap: 5, opacity: 0.5, cursor: "default" }}>
                  <FileText style={{ width: 11, height: 11 }} /> {t("compliance.attestationPdfSoon")}
                </span>
              </>
            </div>
          </div>
        ...
        <Panel>
          <PanelHead title={t("compliance.tokenSupplyBreakdown")} meta={t("compliance.tokenSupplyMeta")} />
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                <th style={{ ...TH, width: "30%" }}>{t("compliance.accountClass")}</th>
                <th style={{ ...TH, textAlign: "right", width: "24%" }}>{t("compliance.balance")}</th>
                <th style={{ ...TH, textAlign: "right", width: "12%" }}>{t("compliance.share")}</th>
            ...
            <tbody>
              ...
              <tr>
                <td style={TD}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: N }}>{t("compliance.totalMinted")}</div>
                  <div style={{ fontSize: 11.5, color: MID, marginTop: 2 }}>{t("compliance.totalMintedDesc")}</div>
                </td>
                <td style={{ ...TD, fontFamily: MONO, fontVariantNumeric: "tabular-nums", textAlign: "right", fontWeight: 600 }}>
                  {ready ? fmtN(total) : t("compliance.na")}
              ...
              <tr>
                <td style={TD}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: N }}>{t("compliance.inCustomerWallets")}</div>
                  <div style={{ fontSize: 11.5, color: MID, marginTop: 2 }}>{t("compliance.inCustomerWalletsDesc")}</div>
                </td>
                <td style={{ ...TD, fontFamily: MONO, fontVariantNumeric: "tabular-nums", textAlign: "right", fontWeight: 600 }}>
                  {ready ? fmtN(circ) : t("compliance.na")}
                  <span style={{ color: MID, fontWeight: 500, fontSize: 11, marginLeft: 4 }}>HTG-C</span>
                </td>
                <td style={{ ...TD, fontFamily: MONO, textAlign: "right", fontWeight: 700, color: N, fontVariantNumeric: "tabular-nums" }}>
                  {ready ? `${circPct.toFixed(3)}%` : t("compliance.na")}
                </td>
                <td style={{ ...TD, paddingRight: 22 }}>
                  <div style={{ height: 6, background: LT, borderRadius: 999, overflow: "hidden" }}>
              ...
              <tr>
                <td style={{ ...TD, borderBottom: "none" }}>
                  <div style={{ fontWeight: 700, fontSize: 13, color: N }}>{t("compliance.distributorFloat")}</div>
                  <div style={{ fontSize: 11.5, color: MID, marginTop: 2 }}>{t("compliance.distributorFloatDesc")}</div>
                </td>
                <td style={{ ...TD, borderBottom: "none", fontFamily: MONO, fontVariantNumeric: "tabular-nums", textAlign: "right", fontWeight: 600 }}>
                  {ready ? fmtN(treas) : t("compliance.na")}
                  <span style={{ color: MID, fontWeight: 500, fontSize: 11, marginLeft: 4 }}>HTG-C</span>
                </td>
                <td style={{ ...TD, borderBottom: "none", fontFamily: MONO, textAlign: "right", fontWeight: 700, color: N, fontVariantNumeric: "tabular-nums" }}>
                  {ready ? `${treasPct.toFixed(3)}%` : t("compliance.na")}
                </td>
          ...
          {ready && (
            <div style={{
              ...
              <span style={{ textTransform: "uppercase", letterSpacing: "0.10em", color: MID }}>{t("compliance.reconciliation")}</span>
              <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                ...
                <span style={{ color: G_FG, fontWeight: 700, display: "flex", alignItems: "center", gap: 5 }}>
                  <CheckCircle2 style={{ width: 13, height: 13 }} /> {t("compliance.balanced")}
                </span>
              </div>
            </div>
          )}
        ...
        <Panel>
          <PanelHead title={t("compliance.regulatoryControls")} meta={t("compliance.regulatoryControlsMeta")} />
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr>
                <th style={{ ...TH, width: "22%" }}>{t("compliance.control")}</th>
                <th style={{ ...TH, width: "44%" }}>{t("compliance.description")}</th>
                <th style={{ ...TH, width: "14%" }}>{t("compliance.status")}</th>
                <th style={{ ...TH, width: "20%" }}>{t("compliance.authority")}</th>
              </tr>
            </thead>
            <tbody>
              {[
                { name: t("compliance.control.kybVerification"),   desc: t("compliance.control.kybVerification.desc"), auth: t("compliance.auth.theoCompliance") },
                { name: t("compliance.control.walletFreeze"),      desc: t("compliance.control.walletFreeze.desc"), auth: t("compliance.auth.theoBrh") },
                { name: t("compliance.control.assetClawback"),     desc: t("compliance.control.assetClawback.desc"), auth: t("compliance.auth.theoLegal") },
                { name: t("compliance.control.amlMonitoring"),     desc: t("compliance.control.amlMonitoring.desc"), auth: t("compliance.auth.theoCompliance") },
                { name: t("compliance.control.counterpartyLimits"), desc: t("compliance.control.counterpartyLimits.desc"), auth: t("compliance.auth.system") },
              ].map((row, i, arr) => (
                <tr key={`${row.name}-${i}`} style={{ background: "transparent" }}>
        ...
        {/* ── 4. SETTLEMENT FLOW ───────────────────────────────────── */}
        <Panel>
          <PanelHead title={t("compliance.settlementAndNetwork")} meta={t("compliance.settlementAndNetworkMeta")} />
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr" }}>
            ...
            <div style={{ padding: "22px 24px" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: N }}>{t("compliance.htgDeposit")}</span>
              </div>
              <div style={{ fontSize: 12, color: MID, margin: "4px 0 16px" }}>{t("compliance.phase1Desc")}</div>
              {[
                { key: t("compliance.phase1.step1"), detail: t("compliance.phase1.step1.detail") },
                { key: t("compliance.phase1.step2"), detail: t("compliance.phase1.step2.detail") },
                { key: t("compliance.phase1.step3"), detail: `${t("compliance.issuer")} ${HTGC_ISSUER.slice(0,4)}…${HTGC_ISSUER.slice(-4)} · ${t("compliance.phase1.step3.detailSuffix")}` },
              ].map((step, i, arr) => (
            ...
            <div style={{ padding: "22px 24px", borderLeft: `1px solid ${LT}` }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 4 }}>
                ...
                <span style={{ fontSize: 14, fontWeight: 700, color: N }}>{t("compliance.usdcRelease")}</span>
              </div>
              <div style={{ fontSize: 12, color: MID, margin: "4px 0 16px" }}>{t("compliance.phase2Desc")}</div>
              {[
                { key: t("compliance.phase2.step1"), detail: t("compliance.phase2.step1.detail") },
                { key: t("compliance.phase2.step2"), detail: `${t("compliance.distributor")} ${HTGC_DISTRIBUTOR.slice(0,4)}…${HTGC_DISTRIBUTOR.slice(-4)} · ${t("compliance.phase2.step2.detailSuffix")}` },
                { key: t("compliance.phase2.step3"), detail: t("compliance.phase2.step3.detail") },
              ].map((step, i) => (
                <div key={i} style={{ display: "flex", gap: 12, alignItems: "flex-start", padding: "12px 0", borderTop: i === 0 ? "none" : `1px dashed ${LT}` }}>
                  <div style={{ width: 22, height: 22, borderRadius: "50%", background: CR, color: N, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 800, flexShrink: 0, marginTop: 1 }}>
                    {i + 1}
        ...
        {/* ── 5. HTG-C ASSET ───────────────────────────────────────── */}
        <Panel>
          <PanelHead title={t("compliance.htgcPanelTitle")} meta={t("compliance.htgcPanelMeta")} />
          <div style={{ padding: "20px 22px" }}>
            <p style={{ fontSize: 13, color: INK, lineHeight: 1.6, margin: 0 }}>
              {t("compliance.htgcParagraph.before")} <strong>Theo</strong>, {t("compliance.htgcParagraph.after")}
            </p>
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }}>
              {[
                { key: t("compliance.chip.backing"),   val: t("compliance.chip.backingHtgcVal"), href: undefined },
                { key: t("compliance.chip.issuer"),    val: t("compliance.chip.issuerHtgcVal"),  href: undefined },
                { key: t("compliance.chip.controls"),  val: t("compliance.chip.controlsVal"),    href: undefined },
                { key: t("compliance.chip.stellar"),   val: `${HTGC_ISSUER.slice(0, 4)}…${HTGC_ISSUER.slice(-4)}`, href: `https://stellar.expert/explorer/testnet/asset/HTGC-${HTGC_ISSUER}` },
              ].map(({ key, val, href }) => {
        ...
        {/* ── 6. USDC COUNTERPARTY ─────────────────────────────────── */}
        <Panel>
          <PanelHead title={t("compliance.usdcPanelTitle")} meta={t("compliance.usdcPanelMeta")} />
          <div style={{ padding: "20px 22px" }}>
            <p style={{ fontSize: 13, color: INK, lineHeight: 1.6, margin: 0 }}>
              {t("compliance.usdcParagraph.before")} <strong>Circle Internet Financial</strong> (NYSE: CRCL), {t("compliance.usdcParagraph.after")}{" "}
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginTop: 14 }}>
              {[
                { key: t("compliance.chip.backing"),   val: t("compliance.chip.backingVal"), href: undefined },
                { key: t("compliance.chip.issuer"),    val: t("compliance.chip.issuerVal"), href: undefined },
                { key: t("compliance.chip.stellar"),   val: "GA5ZSE…KZVN",         href: "https://stellar.expert/explorer/public/asset/USDC-GA5ZSEJYB37JRC5AVCIA5MOP4RHTM335X2KGX3IHOJAPP5RE34K4KZVN" },
              ].map(({ key, val, href }) => {
                const chipStyle: React.CSSProperties = {
                  display: "inline-flex", alignItems: "center", gap: 8,
                  padding: "6px 11px", borderRadius: 999,
