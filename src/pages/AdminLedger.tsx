...
const TREASURY_PUBLIC = "GAO2RZ2T67Z5HJKQHWJXR6TSGEOEOWAAHJBT76DZBAV4W3YUETGMZZMA";
...
export default function AdminLedger() {
  ...
  const [treasuryChain, setTreasuryChain] = useState<number | null>(null);
  ...
  const load = async () => {
    ...
    setLoading(false);
    const [distBal, treasBal] = await Promise.all([
      fetchHorizonBalances(DISTRIBUTOR_PUBLIC),
      fetchHorizonBalances(TREASURY_PUBLIC),
    ]);
    setDistChain(distBal.usdc);
    setTreasuryChain(treasBal.usdc);
  };
  ...
  // ── Reconciliation: book balance for TREASURY_USDC ──
  const treasAccount = accounts.find((a) => a.code === "TREASURY_USDC");
  const treasAgg = treasAccount ? trial.find((t) => t.account.id === treasAccount.id) : null;
  const treasBook = treasAgg ? treasAgg.debit - treasAgg.credit : 0;
  const treasDelta = treasuryChain !== null ? treasuryChain - treasBook : null;
  ...
  return (
    <AppLayout>
      <div style={{ maxWidth: 1200, margin: "0 auto", display: "flex", flexDirection: "column", gap: 20 }}>
        ...
        {/* Reconciliation — Distributor USDC */}
        <div style={card}>
          ...
          <p style={{ fontSize: 12, color: "hsl(var(--theo-mid))", marginTop: 12 }}>
            The distributor (hot) wallet reconciles in real time against Horizon. Any drift here indicates a ledger posting failure — check the Posting Failures panel below.
          </p>
        </div>

        {/* Reconciliation — Treasury USDC */}
        <div style={card}>
          <div style={eyebrow}>Reconciliation — Treasury USDC</div>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 16, marginTop: 8 }}>
            <ReconCell label="Book balance" value={`${fmt(treasBook, "USDC")} USDC`} />
            <ReconCell label="Chain balance (Horizon)" value={treasuryChain === null ? "—" : `${fmt(treasuryChain, "USDC")} USDC`} />
            <ReconCell
              label="Delta (chain − book)"
              value={treasDelta === null ? "—" : `${fmt(treasDelta, "USDC")} USDC`}
              warn={treasDelta !== null && Math.abs(treasDelta) > 1e-7}
            />
          </div>
          <p style={{ fontSize: 12, color: "hsl(var(--theo-mid))", marginTop: 12 }}>
            The treasury (cold) wallet receives USDC from incoming USDC → HTG-C swaps. Drift here indicates leg1 routing or posting disagreement.
          </p>
        </div>
