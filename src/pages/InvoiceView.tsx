...
export default function InvoiceView() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const walletAddress = walletData?.stellar_address ?? null;
      setInvoice({
        ...inv,
      ...
      // Resolve federation address if available (public lookup, no auth needed)
      if (walletAddress) {
        fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/federation?q=${encodeURIComponent(walletAddress)}&type=id`)
          .then((r) => r.ok ? r.json() : null)
          .then((res) => { if (res?.federation_address) setFederationAddress(res.federation_address); })
          .catch(() => {});
      }
    })();
  }, [token]);
                  ...
                  <div style={{ flex: 1, minWidth: 200 }}>
                    <div style={{ marginBottom: 10 }}>
                      ...
                      <div style={{ fontFamily: "monospace", fontSize: 12, color: "#111827", wordBreak: "break-all", background: "#F9FAFB", padding: "8px 10px", borderRadius: 7, border: "1px solid #F3F4F6" }}>
                        {federationAddress ?? invoice.wallet_address}
                      </div>
                      {federationAddress && invoice.wallet_address && (
                        <div style={{ fontSize: 10, color: "#9CA3AF", marginTop: 4 }}>
                          {invoice.wallet_address.slice(0, 6)}…{invoice.wallet_address.slice(-6)}
                        </div>
                      )}
                    </div>
                    <div style={{ marginBottom: 10 }}>
