...
export default function Convert() {
  ...
  // Redirect users who lack convert permission — wait for all auth/permission loading to settle
  useEffect(() => {
    if (authLoading || rolesLoading || permsLoading) return;
    if (!user) return; // ProtectedRoute handles unauthenticated redirect
    if (!isOwner && !isAdmin && !can("convert")) {
      navigate("/dashboard", { replace: true });
    }
  }, [authLoading, rolesLoading, permsLoading, user, isOwner, isAdmin, can, navigate]);
  ...
  const submit = async () => {
    if (!canQuote) { toast.error(t("convert.kybRequired")); return; }
    if (usdcRaw < 1000 || usdcRaw > 52000) { toast.error(t("convert.amountRange")); return; }
    if (!selectedWallet) { toast.error(t("convert.selectDestination")); return; }
    try {
      setBusy(true);
      ...
      if (error || data?.error) { toast.error(data?.error || error?.message || t("convert.quoteFailed")); return; }
      const parsed = parseConversionQuoteResponse(data as Record<string, unknown>);
      if (!parsed) { toast.error(t("convert.invalidQuote")); return; }
      setPendingConversionQuote(parsed);
      toast.success(t("convert.rateLocked").replace("{ref}", String(data.reference_number)));
    } finally {
      setBusy(false);
    }
  };
  ...
  const approveTestKyb = async () => {
    ...
    if (!wallet || !wallet.startsWith("G") || wallet.length < 50) {
      toast.error(t("convert.validAccountRequired"));
      return;
    }
    setBusy(true);
    ...
    setBusy(false);
    setProfile(data as Profile | null);
    toast.success(t("convert.kybApprovedSaved"));
  };
  ...
  const handleAddBank = async () => {
    if (!addBankName || !addAccountName || !addAccountNumber) {
      toast.error(t("convert.fillRequired"));
      return;
    }
    setAddBankBusy(true);
    const { data: au } = await supabase.auth.getUser();
    const cid = await resolveCustomerId(au.user?.id ?? "");
    if (!cid) { toast.error(t("convert.customerNotFound")); setAddBankBusy(false); return; }
    const c = { id: cid };
    ...
    toast.success(t("convert.bankAdded"));
  ...
  const handleWithdraw = async () => {
    ...
    if (!bank) { toast.error(t("convert.selectDestinationBank")); return; }
    if (offAmountRaw < 100) { toast.error(t("convert.minWithdrawal")); return; }
    const wallet = walletOptions.find((w) => w.id === offSourceWallet)
      ?? walletOptions.find((w) => w.stellar_address === selectedWallet)
      ?? walletOptions[0];
    if (!wallet) { toast.error(t("convert.noWalletSelected")); return; }
    setOffBusy(true);
    try {
      const { data: au } = await supabase.auth.getUser();
      const cid = await resolveCustomerId(au.user?.id ?? "");
      if (!cid) { toast.error(t("convert.customerNotFound")); return; }
      const c = { id: cid };
      ...
      if (error || data?.error) {
        toast.error(data?.error || error?.message || t("convert.withdrawFailed"));
        return;
      }
      setOffConfirm(false);
      toast.success(t("convert.withdrawSubmitted"));
  ...
  const handleHtgSubmit = async () => {
    if (htgAmountRaw < 1) { toast.error(t("convert.enterAmount")); return; }
    if (!selectedWallet) { toast.error(t("convert.selectDestination")); return; }

    if (htgReceiveMode === "usdc") {
      if (!canQuote) { toast.error(t("convert.kybToUsdcRequired")); return; }
      if (!liveRate) { toast.error(t("convert.rateUnavailable")); return; }
      const f = totalBps / 10_000;
      const denom = 1 - f;
      ...
      if (usdcGrossRounded < 1000 || usdcGrossRounded > 52000) {
        toast.error(t("convert.amountRange"));
        return;
      }
      setHtgBusy(true);
      try {
        ...
        if (error || data?.error) {
          toast.error(data?.error || error?.message || t("convert.createQuoteFailed"));
          return;
        }
        const parsed = parseConversionQuoteResponse(data as Record<string, unknown>);
        if (!parsed) { toast.error(t("convert.invalidQuote")); return; }
    ...
    try {
      ...
      if (error || data?.error) {
        toast.error(data?.error || error?.message || t("convert.depositRefFailed"));
        return;
      }
      toast.success(t("convert.depositRef").replace("{ref}", String(data.reference_number)));
  ...
  const handleSwapSubmit = () => {
    if (swapAmountRaw < 1) { toast.error(t("convert.enterAmount")); return; }
  ...
  const handleSwapConfirm = async () => {
    ...
    try {
      ...
      if (error || data?.error) {
        if (data?.refunded) {
          toast.error(t("convert.swapReturned"), {
            description: data?.detail ? String(data.detail).slice(0, 160) : undefined,
          });
        } else if (data?.refundFailed) {
          toast.error(t("convert.swapHeld"), {
            description: t("convert.swapHeldDesc"),
          });
        } else {
          toast.error(data?.error || error?.message || t("convert.swapFailed"));
        }
        if (selectedWallet) fetchHorizonBalances(selectedWallet).then(setWalletBalances);
        return;
      }
      toast.success(t("convert.swapCompleted"));
      if (selectedWallet) fetchHorizonBalances(selectedWallet).then(setWalletBalances);
  ...
  return (
    <AppLayout>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "3fr 2fr" }}>
        ...
        <div className="bg-card border border-border rounded-xl p-5 shadow-xs">
          <div className="flex border-b border-border mb-4" style={{ overflowX: "auto" }}>
            <button style={tabStyle("htg")} onClick={() => setTab("htg")}>{t("convert.tab.depositHtg")}</button>
            <button style={tabStyle("swap")} onClick={() => setTab("swap")}>{t("convert.tab.swap")}</button>
            <button style={tabStyle("off")} onClick={() => setTab("off")}>{t("convert.tab.withdrawBank")}</button>
            <button style={tabStyle("wire")} onClick={() => setTab("wire")}>{t("convert.tab.globalWire")}</button>
            ...
            <>
              ...
              <div className="rounded-xl mb-4 flex items-start gap-2.5" style={{ background: "hsl(var(--theo-blue-soft))", border: "1px solid hsl(var(--theo-blue-chip))", padding: "12px 14px" }}>
                ...
                <div style={{ fontSize: 12, color: "hsl(var(--theo-blue))", lineHeight: 1.6 }}>
                  {htgReceiveMode === "htgc"
                    ? <>{t("convert.htgBannerMint.before")} <strong>SPIH</strong>. {t("convert.htgBannerMint.after")}</>
                    : <>{t("convert.htgBannerAutoConvert.before")} <strong>SPIH</strong>. {t("convert.htgBannerAutoConvert.after")}</>}
                </div>
              </div>
              ...
              <div style={{ marginBottom: 14 }}>
                <div className="flex items-center justify-between gap-2 mb-1.5">
                  <label style={{ ...labelStyle, marginBottom: 0 }}>{t("convert.you.receive")}</label>
                  <span
                    title={t("convert.availableInWallet").replace("{asset}", htgReceiveMode === "usdc" ? "USDC" : "HTG-C")}
                    style={{
                  ...
                  >
                    {t("convert.availableLabel").replace(
                      "{amount}",
                      htgReceiveMode === "usdc"
                    ...
                    ).replace("{asset}", htgReceiveMode === "usdc" ? "USDC" : "HTG-C")}
                  </span>
                </div>
                <div className="flex items-center gap-2">
                  {dirToggle(htgReceiveMode === "usdc", () => setHtgReceiveMode("usdc"), t("convert.receiveMode.usdc"))}
                  {dirToggle(htgReceiveMode === "htgc", () => setHtgReceiveMode("htgc"), t("convert.receiveMode.htgc"))}
                </div>
              </div>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={labelStyle}>{t("convert.wallet")}</label>
              ...
              <div className="rounded-xl mb-4" style={{ background: "hsl(var(--theo-blue-soft))", border: "1px solid hsl(var(--theo-blue-chip))", padding: "14px 16px" }}>
                <div className="flex justify-between mb-1.5">
                  <span style={{ fontSize: 12, color: "hsl(var(--theo-mid))" }}>{t("convert.you.send")}</span>
                  <span style={{ fontSize: 15, fontWeight: 800, color: "hsl(var(--theo-blue))" }}>{htgAmount || "0"} HTG</span>
                </div>
                {htgReceiveMode === "htgc" ? (
                  <>
                    <div className="flex justify-between mb-1.5">
                      <span style={{ fontSize: 12, color: "hsl(var(--theo-mid))" }}>{t("convert.you.receive")}</span>
                      <span style={{ fontSize: 15, fontWeight: 800, color: "hsl(var(--theo-blue))" }}>{htgAmount || "0"} HTG-C</span>
                    </div>
                    <div className="flex justify-between mb-1.5">
                      <span style={{ fontSize: 12, color: "hsl(var(--theo-mid))" }}>{t("convert.peg")}</span>
                      <span style={{ fontSize: 13, fontWeight: 700, color: "hsl(var(--theo-blue))" }}>{t("convert.pegGuaranteed")}</span>
                    </div>
                    <div className="flex justify-between">
                      <span style={{ fontSize: 12, color: "hsl(var(--theo-mid))" }}>{t("convert.settlement")}</span>
                      <span style={{ fontSize: 13, fontWeight: 700, color: "hsl(var(--theo-blue))" }}>{t("convert.autoSameBusinessDay")}</span>
                    </div>
                  ...
                  <>
                    {(() => {
                      ...
                      return (
                        <>
                          <div className="flex justify-between mb-1.5">
                            <span style={{ fontSize: 12, color: "hsl(var(--theo-mid))" }}>{t("common.rate")}</span>
                          ...
                          <div className="flex justify-between mb-1.5">
                            <span style={{ fontSize: 12, color: "hsl(var(--theo-mid))" }}>{t("convert.net")}</span>
                          ...
                          <div className="flex justify-between">
                            <span style={{ fontSize: 12, color: "hsl(var(--theo-mid))" }}>{t("convert.quoteLock")}</span>
                            <span style={{ fontSize: 13, fontWeight: 700, color: "hsl(var(--theo-blue))" }}>{t("convert.quoteLockDuration")}</span>
                          </div>
                        </>
                      );
                    })()}
              ...
              >
                {htgBusy
                  ...
                  : htgReceiveMode === "usdc" ? t("convert.lockRateAndGetRef") : t("convert.getDepositRef")}
              </button>
            </>
          )}
          ...
          {tab === "swap" && (
            <>
                ...
                return (
                  <div style={{ marginBottom: 14 }}>
                    ...
                    <div className="flex items-center justify-between mb-1.5">
                      ...
                      <span
                        title={t("convert.availableInWallet").replace("{asset}", currency)}
                        style={{
                          fontSize: 11, fontWeight: 600,
                          color: "hsl(var(--theo-blue))",
                          background: "hsl(var(--theo-blue-soft))",
                      ...
                      >
                        {balLoading
                          ? "..."
                          : t("convert.availableLabel")
                              .replace(
                                "{amount}",
                                availBal > 0
                                  ? (Math.floor(availBal * 100) / 100).toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 })
                                  : "0.00",
                              )
                              .replace("{asset}", currency)}
                      </span>
                    </div>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={labelStyle}>{t("balance.table.account")}</label>
                {walletOptions.length === 0 ? (
                  <div style={{ ...inputStyle, display: "flex", alignItems: "center", color: "hsl(var(--theo-mid))", fontSize: 13 }}>
                    No accounts yet — create one on the Balance page.
                  </div>
                ...
                <div className="flex items-center justify-between mt-2.5 pt-2.5" style={{ borderTop: "1px solid hsl(var(--theo-blue-chip))" }}>
                  <div className="flex items-center gap-1.5">
                    ...
                    <span style={{ fontSize: 11, color: "hsl(var(--theo-cyan))", fontWeight: 600 }}>
                      {t("convert.rate.locked")} <strong>{timerLabel}</strong>
                    </span>
                  </div>
                  <span style={{ fontSize: 11, color: "hsl(var(--theo-mid))" }}>{t("convert.rate.brhOfficial")}</span>
                </div>
              </div>
              ...
              >
                {swapBusy ? t("convert.swapping") : swapDir === "htgc_to_usdc" ? t("convert.swapCta.htgcToUsdc") : t("convert.swapCta.usdcToHtgc")}
              </button>
            </>
          )}
            ...
            <>
              ...
              <div style={{ marginBottom: 14 }}>
                <div className="flex items-center justify-between" style={{ marginBottom: 6 }}>
                  <label style={{ ...labelStyle, marginBottom: 0 }}>{t("convert.htgcToRedeem")}</label>
                  <span
                    style={{
                  ...
                  >
                    {t("convert.availableLabel")
                      .replace(
                        "{amount}",
                        offHtgcLoading || offHtgcBalance == null
                          ? "—"
                          : Math.floor(offHtgcBalance).toLocaleString("en-US", { minimumFractionDigits: 0, maximumFractionDigits: 0 }),
                      )
                      .replace("{asset}", "HTG-C")}
                  </span>
                </div>
              ...
              {/* Source wallet */}
              <div style={{ marginBottom: 14 }}>
                <label style={labelStyle}>{t("payout.sourceAccount")}</label>
                <select
                ...
                >
                  {walletOptions.length === 0
                    ? <option>{t("convert.noAccountsYet")}</option>
              ...
              <div style={{ marginBottom: 14 }}>
                <label style={labelStyle}>{t("convert.destinationBank")}</label>
                {bankLoading ? (
                  <div style={{ ...inputStyle, color: "hsl(var(--theo-mid))", fontSize: 13 }}>{t("common.loading")}</div>
                  ...
                  >
                    <Plus size={14} /> {t("convert.addBankAccount")}
                  </button>
                ) : (
                ...
                return (
                  <div className="rounded-xl mb-4 p-4" style={{ background: "hsl(var(--theo-cream))", border: "1px solid hsl(var(--theo-light))" }}>
                    <div style={{ fontSize: 11, fontWeight: 700, letterSpacing: "0.18em", textTransform: "uppercase", color: "hsl(var(--theo-cyan))", marginBottom: 10 }}>
                      {t("convert.transferSummary")}
                    </div>
                    <div className="flex justify-between" style={{ fontSize: 12, marginBottom: 6 }}>
                      <span style={{ color: "hsl(var(--theo-mid))" }}>{t("convert.redemptionAmount")}</span>
                      <span style={{ fontWeight: 700, color: "hsl(var(--theo-blue))" }}>{fmt(redemption)} HTG-C</span>
                    </div>
                    <div className="flex justify-between" style={{ fontSize: 12, marginBottom: 6 }}>
                      <span style={{ color: "hsl(var(--theo-mid))" }}>{t("convert.spihFee")}</span>
                      <span style={{ fontWeight: 700, color: "#1A7F37" }}>{t("convert.includedFree")}</span>
                    </div>
                    <div className="flex justify-between" style={{ fontSize: 12, marginBottom: 6 }}>
                      <span style={{ color: "hsl(var(--theo-mid))" }}>{t("convert.theoServiceFeeZeroBps")}</span>
                      <span style={{ fontWeight: 700, color: "#1A7F37" }}>$0.00</span>
                    </div>
                    <div className="flex justify-between items-center gap-2" style={{ fontSize: 13, marginTop: 8, paddingTop: 8, borderTop: "1px solid hsl(var(--theo-light))" }}>

                      <span className="flex items-center gap-1.5" style={{ fontWeight: 800, color: "hsl(var(--theo-blue))" }}>
                        {t("convert.recipientReceives")}
                        <TooltipProvider>
                          <Tooltip>
                            ...
                            <TooltipContent side="top" style={{ maxWidth: 240, fontSize: 11 }}>
                              {t("convert.localBankTooltip")}
                            </TooltipContent>
                          </Tooltip>
                        </TooltipProvider>
                      </span>
                      <span style={{ fontWeight: 800, color: "hsl(var(--theo-blue))" }}>
                        <span style={{ color: "hsl(var(--theo-cyan))" }}>{fmt(totalSettlement)}</span> HTG
                      </span>
                    </div>
                  </div>
                );
              ...
              <div className="rounded-xl mb-4 flex items-start gap-2.5" style={{ background: "hsl(var(--theo-blue-soft))", border: "1px solid hsl(var(--theo-blue-chip))", padding: "12px 14px" }}>
                ...
                <div style={{ fontSize: 12, color: "hsl(var(--theo-blue))", lineHeight: 1.6 }}>
                  {t("convert.withdrawNote.before")} <strong>SPIH</strong>. {t("convert.withdrawNote.after")} <strong>{t("convert.withdrawNote.timeline")}</strong>.
                </div>
              </div>
              ...
              <>
                ...
                <div className="rounded-xl mb-4 flex items-start gap-2.5" style={{ background: "hsl(var(--theo-cyan) / 0.08)", border: "1.5px solid hsl(var(--theo-cyan))", padding: "12px 14px" }}>
                  ...
                  <div style={{ fontSize: 12, color: "hsl(var(--theo-blue))", lineHeight: 1.6, fontWeight: 600 }}>
                    {t("convert.wire.title")} <span style={{ opacity: 0.75, fontWeight: 500 }}>({t("convert.wire.bypassesSpih")})</span>
                  </div>
                </div>
                ...
                <div style={{ marginBottom: 12 }}>
                  <div className="flex items-center justify-between" style={{ marginBottom: 6 }}>
                    <label style={{ ...labelStyle, marginBottom: 0 }}>{t("convert.wire.sourceAccount")} <span style={{ color: "#C00" }}>*</span></label>
                    {selectedWireWallet && (
                ...
                <div style={{ marginBottom: 12 }}>
                  <label style={labelStyle}>{t("convert.wire.recipientFullName")} <span style={{ color: "#C00" }}>*</span></label>
                ...
                <div className="grid gap-3" style={{ gridTemplateColumns: "1fr 1fr", marginBottom: 12 }}>
                  ...
                  <div>
                    <label style={labelStyle}>{t("convert.wire.bankCityCountry")} <span style={{ color: "#C00" }}>*</span></label>
                ...
                <div className="rounded-xl mb-4 p-4" style={{ background: "hsl(var(--theo-cream))", border: "1px solid hsl(var(--theo-light))" }}>
                  ...
                  <div className="flex justify-between" style={{ fontSize: 12, marginBottom: 6 }}>
                    <span style={{ color: "hsl(var(--theo-mid))" }}>{t("convert.wire.principal")}</span>
                    <span style={{ fontWeight: 700, color: "hsl(var(--theo-blue))" }}>${fmt(wireAmountRaw)} USDC</span>
                  </div>
                  <div className="flex justify-between" style={{ fontSize: 12, marginBottom: 6 }}>
                    <span style={{ color: "hsl(var(--theo-mid))" }}>{t("convert.wire.bankWireFee")} <span style={{ opacity: 0.7 }}>({t("convert.wire.flat")})</span></span>
                    <span style={{ fontWeight: 700, color: "hsl(var(--theo-blue))" }}>${fmt(WIRE_FLAT_FEE)} USDC</span>
                  </div>
                  <div className="flex justify-between" style={{ fontSize: 12, marginBottom: 6 }}>
                    <span style={{ color: "hsl(var(--theo-mid))" }}>{t("convert.wire.processingFee")} <span style={{ opacity: 0.7 }}>({(variableBps / 100).toFixed(2)}%)</span></span>
                  ...
                  <div className="flex justify-between" style={{ fontSize: 13, marginTop: 8, paddingTop: 8, borderTop: "1px solid hsl(var(--theo-light))" }}>
                    <span style={{ fontWeight: 700, color: "hsl(var(--theo-blue))" }}>{t("convert.wire.totalDeducted")}</span>
                  ...
                  <div className="flex justify-between items-center" style={{ fontSize: 13, marginTop: 10, paddingTop: 10, borderTop: "1px dashed hsl(var(--theo-cyan))" }}>
                    <span style={{ fontWeight: 800, color: "hsl(var(--theo-blue))" }}>{t("convert.wire.netDelivery")}</span>
                    <span style={{ fontWeight: 800, color: "hsl(var(--theo-blue))" }}>
                      {t("convert.wire.destinationAccountWillReceive")} <span style={{ color: "hsl(var(--theo-cyan))" }}>${fmt(netDelivered)}</span> {t("convert.wire.inLocalCurrency")}
                    </span>
                  </div>
                </div>
                ...
                <div className="rounded-xl mb-4 flex items-start gap-2.5" style={{ background: "hsl(var(--theo-blue-soft))", border: "1px solid hsl(var(--theo-blue-chip))", padding: "12px 14px" }}>
                  <Info className="shrink-0" style={{ width: 14, height: 14, color: "hsl(var(--theo-blue))", marginTop: 2 }} />
                  <div style={{ fontSize: 12, color: "hsl(var(--theo-blue))", lineHeight: 1.6 }}>
                    {t("convert.wire.standardSettlement")} <strong>{t("convert.wire.oneToThreeBusinessDays")}</strong>. {t("convert.wire.poweredByLiquidityProviders")}
                  </div>
                </div>
        ...
        <div className="flex flex-col gap-3">
          <div className="bg-card border border-border rounded-xl p-5 shadow-xs">
            <div className="font-bold mb-3" style={{ fontSize: 13, color: "hsl(var(--theo-blue))" }}>{t("convert.corridor.title")}</div>
            <p style={{ fontSize: 12, color: "hsl(var(--theo-mid))", lineHeight: 1.6, marginBottom: 12 }}>
              {t("convert.corridor.body")}
            </p>
            {[
              [t("convert.corridor.avgSettlement"), t("convert.corridor.avgSettlement.value")],
              [t("convert.corridor.network"), t("convert.corridor.network.value")],
              [t("convert.corridor.reserveModel"), t("convert.corridor.reserveModel.value")],
              [t("convert.corridor.maxPerOrder"), t("convert.corridor.maxPerOrder.value")],
            ].map(([k, v]) => (
          ...
          <div className="bg-card border border-border rounded-xl p-5 shadow-xs">
            <div className="flex items-center justify-between mb-2">
              <div className="font-bold" style={{ fontSize: 13, color: "hsl(var(--theo-blue))" }}>{t("convert.liveRate")}</div>
              {rateSource === "brh" && (
                <a
            ...
            <div style={{ fontSize: 11, color: "hsl(var(--theo-mid))", marginTop: 2 }}>{t("convert.htgPerUsdc")}</div>
            <div className="flex items-center gap-1.5 mt-1.5">
              <div className="rounded-full" style={{ width: 6, height: 6, background: "hsl(var(--theo-cyan))", animation: "pulse 2s infinite" }} />
              <span style={{ fontSize: 11, color: "hsl(var(--theo-cyan))", fontWeight: 600 }}>
                {rateSource === "brh" ? t("convert.rate.brhDaily") : t("convert.rate.live5s")}
              </span>
            </div>
            {rateCapturedAt && rateSource === "brh" && (
              <div style={{ fontSize: 10, color: "hsl(var(--theo-mid))", marginTop: 4, opacity: 0.7 }}>
                {t("convert.asOf")} {new Date(rateCapturedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
              </div>
            )}
          </div>
          ...
          <div className="rounded-xl p-5 shadow-xs" style={{ background: "#EFFBF3" }}>
              ...
              <TooltipProvider delayDuration={150}>
                <Tooltip>
                  <TooltipTrigger asChild>
                    <button
                      type="button"
                      aria-label={t("convert.lifetimeSavings.how")}
                  ...
                  <TooltipContent side="top" className="max-w-xs text-xs">
                    {t("convert.lifetimeSavings.tooltip")}
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
            </div>
            ...
            <div style={{ fontSize: 11, fontWeight: 600, color: "hsl(150 30% 30%)", marginTop: 6 }}>
              {t("convert.lifetimeSavings.compared")}
            </div>
          </div>
        </div>
      </div>
      ...
      {pendingConversionQuote && (() => {
        ...
        const rows: [string, string][] = [
          [t("convert.confirm.reference"), q.referenceNumber],
          [t("convert.confirm.rate"), `${q.rate.toFixed(2)} HTG / USDC`],
          [t("convert.confirm.htgSent"), `${Math.round(q.htgRequired).toLocaleString("en-US")} HTG`],
          [t("convert.confirm.usdcGross"), fmtUsdc2dp(q.usdcGross)],
          [t("convert.confirm.theoFee"), `${fmtUsdc2dp(q.feeUsdc)}${feePct}`],
          [t("convert.confirm.usdcReceived"), fmtUsdc2dp(q.usdcAmount)],
        ];
        return (
          <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.45)", zIndex: 50, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
            <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 420, boxShadow: "0 20px 60px rgba(0,0,0,0.2)", overflow: "hidden" }}>
              <div className="px-5 py-4" style={{ borderBottom: "1px solid hsl(var(--theo-light))" }}>
                <div className="font-bold" style={{ fontSize: 15, color: "hsl(var(--theo-blue))" }}>{t("convert.confirm.title")}</div>
                <div style={{ fontSize: 12, color: "hsl(var(--theo-mid))", marginTop: 1 }}>{t("convert.confirm.subtitle")}</div>
              </div>
              ...
              <div className="p-5">
                ...
                <div className="flex gap-2 mt-4">
                  ...
                  >
                    <CheckCircle2 size={14} /> {t("convert.confirm.lock")}
                  </button>
                </div>
              </div>
            </div>
