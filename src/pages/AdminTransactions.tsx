...
import {
  ...
  Download, ChevronDown, ChevronRight, ChevronLeft, Repeat, Coins,
} from "lucide-react";
...
const KIND_META: Record<string, KindMeta> = {
  ...
  HTGC_WITHDRAWAL:      { label: "HTG withdrawal · bank",      direction: "out" },
...
// orders.order_kind → admin label/direction, for orders surfaced directly
// (not via the ledger). Every kind here is customer-impacting (never treasury-
// hidden) so the admin log stays a complete "god view".
const ORDER_KIND_META: Record<string, KindMeta> = {
  usdc_conversion: { label: "HTG → USDC conversion",     direction: "internal" },
  htgc_mint:       { label: "Deposit · HTG-C delivered", direction: "in" },
  htgc_usdc_swap:  { label: "HTG-C → USDC swap",         direction: "internal" },
  htgc_withdrawal: { label: "HTG withdrawal · bank",     direction: "out" },
};
function orderMetaFor(kind: string): KindMeta {
  return ORDER_KIND_META[kind] ?? { label: kind || "Order", direction: "internal" };
}
...
export default function AdminTransactions() {
  ...
  async function load(before?: string) {
    ...
    try {
      ...
      // ── Merge in raw payouts ───────────────────────────────────────
      // The admin log is ledger-sourced, but not every payout posts a ledger
      // entry (seed data, or payouts that bypassed the ledger path). Surface
      // them so an admin can answer "did my payment go through?" for any
      // client payout. Dedupe against payouts already booked to the ledger
      // by stellar_tx_hash. First page only (avoids paginated duplicates).
      let payoutRows: Row[] = [];
      if (!before) {
        const { data: payoutData } = await supabase
          .from("payouts")
          .select("id, customer_id, recipient_name, amount_usdc, memo, status, stellar_tx_hash, created_at")
          .order("created_at", { ascending: false });
        const payList = (payoutData ?? []) as Array<{
          id: string; customer_id: string; recipient_name: string;
          amount_usdc: number; memo: string | null; status: string;
          stellar_tx_hash: string | null; created_at: string;
        }>;
        const extraPayoutCustomerIds = Array.from(new Set(
          payList.map(p => p.customer_id).filter(cid => cid && !customerMap.has(cid)),
        ));
        if (extraPayoutCustomerIds.length) {
          const { data: extra } = await supabase
            .from("customers").select("id, company_name, email").in("id", extraPayoutCustomerIds);
          (extra ?? []).forEach((c: Customer) => { customerList.push(c); customerMap.set(c.id, c); });
        }
        const ledgerPayoutHashes = new Set(
          txList.filter(t => t.kind === "PAYOUT_USDC" || t.kind === "USDC_PAYOUT")
                .map(t => t.stellar_tx_hash).filter(Boolean) as string[],
        );
        payoutRows = payList
          .filter(p => !p.stellar_tx_hash || !ledgerPayoutHashes.has(p.stellar_tx_hash))
          .map(p => ({
            tx: {
              id: `payout:${p.id}`,
              kind: "PAYOUT_USDC",
              description: `USDC payment to ${p.recipient_name}${p.memo ? " · " + p.memo : ""}`,
              order_id: null,
            ...
            customer: customerMap.get(p.customer_id) ?? null,
            htg: 0,
            usdc: Number(p.amount_usdc),
            meta: metaFor("PAYOUT_USDC"),
            statusOverride: p.status,
          }));
      }

      // ── Merge in ANY order not booked to the ledger ────────────────
      // The admin log is the "god view": every customer-impacting movement
      // must appear, even if it never posted a ledger entry (seed data, or
      // a path that bypassed the ledger). Pull all orders and surface any
      // whose id isn't already represented by a ledger transaction. Covers
      // conversions, deposits/mints, swaps, AND bank withdrawals uniformly.
      // Dedupe by order_id. First page only.
      let orderRows: Row[] = [];
      if (!before) {
        const { data: oData } = await supabase
          .from("orders")
          .select("id, reference_number, order_kind, htg_amount, usdc_amount, status, customer_id, created_at, stellar_tx_hash")
          .order("created_at", { ascending: false });
        const oList = (oData ?? []) as Array<{
          id: string; reference_number: string | null; order_kind: string;
          htg_amount: number | null; usdc_amount: number | null; status: string;
          customer_id: string; created_at: string; stellar_tx_hash: string | null;
        }>;
        const extraOCustomerIds = Array.from(new Set(
          oList.map(o => o.customer_id).filter(cid => cid && !customerMap.has(cid)),
        ));
        if (extraOCustomerIds.length) {
          const { data: extra } = await supabase
            .from("customers").select("id, company_name, email").in("id", extraOCustomerIds);
          (extra ?? []).forEach((c: Customer) => { customerList.push(c); customerMap.set(c.id, c); });
        }
        const ledgerOrderIds = new Set(txList.map(t => t.order_id).filter(Boolean) as string[]);
        orderRows = oList
          .filter(o => !ledgerOrderIds.has(o.id))
          .map(o => {
            const m = orderMetaFor(o.order_kind);
            return {
              tx: {
                id: `order:${o.id}`,
                kind: o.order_kind,
                description: `${m.label}${o.reference_number ? " · " + o.reference_number : ""}`,
                order_id: o.id,
              ...
              htg: Number(o.htg_amount ?? 0),
              usdc: Number(o.usdc_amount ?? 0),
              meta: m,
              statusOverride: o.status,
            };
          });
      }

      const merged = [...newRows, ...blendRows, ...payoutRows, ...orderRows].sort(
  ...
  useEffect(() => { setPage(1); }, [range, customerId, kindLabel, query, showTreasury, perPage]);
  ...
  function exportCsv() {
    const header = [
      "created_at", "transaction_id", "kind", "label", "direction", "status",
      "customer", "email", "customer_id", "order_id", "reference", "description",
      "htg", "usdc", "stellar_tx_hash",
    ];
    ...
    filtered.forEach(r => {
      const cells = [
        ...
        r.meta.label,
        r.meta.direction,
        r.statusOverride ?? r.order?.status ?? "POSTED",
        r.customer?.company_name ?? "",
        r.customer?.email ?? "",
        r.customer?.id ?? r.order?.customer_id ?? "",
        r.tx.order_id ?? r.order?.id ?? "",
        r.order?.reference_number ?? "",
        r.tx.description ?? "",
    ...
    // BOM for clean UTF-8 import into Excel/Sheets
                  ...
                  return (
                    <React.Fragment key={r.tx.id}>
                      <tr key={r.tx.id} onClick={() => setExpanded(isOpen ? null : r.tx.id)}
                        ...
                        <td style={td}><StatusBadge status={r.statusOverride ?? r.order?.status ?? "POSTED"} /></td>
                      </tr>
                      {isOpen && (
