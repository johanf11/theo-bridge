...
// Module-level formatters — pinned to NUMBER_LOCALE (en-US), no hook needed
const fmt  = (n: number) => n.toLocaleString(NUMBER_LOCALE, { minimumFractionDigits: 2, maximumFractionDigits: 2 });
const fmt4 = (n: number) => n.toLocaleString(NUMBER_LOCALE, { minimumFractionDigits: 4, maximumFractionDigits: 4 });
...
export default function Balance() {
  ...
  // Owners and admins always see balances; members need the view_balances permission
  ...
  const [federationAddresses, setFederationAddresses] = useState<Record<string, string>>({});
  ...
  const loadWallets = async () => {
    ...
    // Resolve federation addresses for each wallet (fire-and-forget, no auth needed)
    const fedBase = `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/federation`;
    const fedEntries = await Promise.all(
      ws.map(async (x) => {
        try {
          const res = await fetch(`${fedBase}?q=${encodeURIComponent(x.stellar_address)}&type=id`);
          const json = res.ok ? await res.json() : null;
          return [x.id, json?.federation_address ?? null] as [string, string | null];
        } catch {
          return [x.id, null] as [string, null];
        }
      })
    );
    setFederationAddresses(
      Object.fromEntries(fedEntries.filter(([, v]) => v !== null)) as Record<string, string>
    );
  };
  ...
  const moveSourceLabel = wallets.find((w) => w.id === moveSourceId)?.label ?? t("balance.move.source");
  const moveDestLabel = wallets.find((w) => w.id === moveDestId)?.label ?? t("balance.move.destination");
    ...
    <AppLayout>
      ...
      <div className="flex items-start justify-between mb-1">
        ...
        <div className="flex gap-2">
          {can("payout_send") && wallets.length >= 2 && (
            ...
            >
              <ArrowLeftRight size={12} /> {t("balance.moveFunds")}
            </button>
          )}
          <button
      ...
      <div className="flex items-center justify-between mb-3" style={{ background: "hsl(var(--theo-blue))", borderRadius: 14, padding: "24px 28px" }}>
        <div>
          <div className="font-bold uppercase mb-2" style={{ fontSize: 10, letterSpacing: "0.14em", color: "hsl(var(--theo-gold))" }}>
            {t("balance.totalAcross")}
          </div>
          <div className="font-extrabold leading-none" style={{ fontSize: 40, letterSpacing: "-2px", color: "#fff" }}>
            {canViewBalances ? `$${fmt(Number(total + totalEarning))}` : "—"}
          </div>
          <div style={{ fontSize: 13, color: "rgba(255,255,255,0.50)", marginTop: 4 }}>
            {t("balance.totalSubline")}
            {canViewBalances && hasPositions && <span style={{ color: "rgba(255,255,255,0.4)", marginLeft: 8 }}>{t("balance.includingEarning").replace("{amount}", fmt(Number(totalEarning)))}</span>}
          </div>
        </div>
        <div className="flex items-center gap-1.5">
          <div className="rounded-full" style={{ width: 6, height: 6, background: "hsl(var(--theo-cyan))", animation: "pulse 2s infinite" }} />
          <span className="font-semibold" style={{ fontSize: 12, color: "hsl(var(--theo-cyan))" }}>{t("balance.liveVerified")}</span>
        </div>
      </div>
      ...
      {hasPositions ? (
        <div className="rounded-2xl mb-4 overflow-hidden" style={{ border: "1.5px solid #86EFAC", background: "#F0FDF4" }}>
          ...
          <div className="grid grid-cols-3 divide-x divide-[#BBF7D0]" style={{ borderBottom: "1px solid #BBF7D0" }}>
            {[
              { label: t("balance.stats.earning"), value: canViewBalances ? `$${fmt(totalEarning)}` : "—", sub: t("balance.stats.earningSub") },
              { label: t("balance.stats.today"), value: canViewBalances ? `+$${fmt(totalAccruedToday)}` : "—", sub: t("balance.stats.todaySub") },
              { label: t("balance.stats.month"), value: canViewBalances ? `+$${fmt(totalAccruedMonth)}` : "—", sub: t("balance.stats.monthSub") },
            ].map((s) => (
              ...
              <div key={pos.walletId} className="flex items-center justify-between gap-4 rounded-xl px-4 py-3" style={{ background: "#DCFCE7", border: "1px solid #BBF7D0" }}>
                <div>
                  ...
                  <div style={{ fontSize: 11, color: "#166534", marginTop: 1 }}>
                    {canViewBalances
                      ? t("balance.positionLine")
                          .replace("{deposited}", fmt(pos.deposited))
                          .replace("{accrued}", fmt(pos.accrued))
                          .replace("{daily}", fmt(dailyYield(pos.deposited)))
                      : "—"}
                  </div>
                </div>
                <button
                ...
                >
                  {blendWithdrawing && blendWithdrawPos?.walletId === pos.walletId
                    ? <><Loader2 size={11} className="animate-spin" /> {t("balance.withdrawing")}</>
                    : <><ArrowUpFromLine size={11} /> {t("balance.withdraw")}</>}
                </button>
              </div>
            ))}
          </div>
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="font-bold flex items-center gap-1.5" style={{ fontSize: 14, color: "#14532D" }}>
                {t("balance.earnIdle").replace("{apy}", (NET_APY * 100).toFixed(2))}
                <span
                ...
                {wallets.map((w, i) => {
                  ...
                  {canDeleteWallet && (
                    <button
                      ...
                      title={t("balance.deleteEmptyWallet")}
                      style={{
                  ...
                  <div className="font-bold uppercase mb-2.5" style={{ fontSize: 10, letterSpacing: "0.12em", color: "rgba(255,255,255,0.50)" }}>
                    {editingId === w.id ? (
                      ...
                      <span
                        ...
                        title={can("accounts_manage") ? t("balance.renameHoldDrag") : t("balance.holdDrag")}
                  ...
                  {!displayHtgcZero && (
                    <div className="flex items-center gap-1.5 mt-2.5 pt-2.5" style={{ borderTop: "1px solid rgba(255,255,255,0.12)" }}>
                      <span style={{ fontSize: 16, fontWeight: 800, color: "hsl(var(--theo-gold))", letterSpacing: "-0.5px" }}>
                        {canViewBalances ? Math.round(htgc).toLocaleString(NUMBER_LOCALE) : "—"}
                      </span>
                  ...
                  {pos ? (
                    <div className="flex items-center gap-1.5 mt-2" style={{ height: 16, fontSize: 11, color: "#4ADE80", fontWeight: 600 }}>
                      <TrendingUp size={11} />
                      {t("balance.cardEarning")
                        .replace("{amount}", fmt(pos.deposited + pos.accrued))
                        .replace("{daily}", fmt(dailyYield(pos.deposited)))}
                    </div>
                  ) : (
                  ...
                  <div className="flex items-center gap-1.5 mt-3">
                    <div className="rounded-full flex-shrink-0" style={{ width: 6, height: 6, background: "hsl(var(--theo-cyan))" }} />
                    {canViewBalances ? (
                      <a
                      ...
                      >
                        <span style={{ overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                          {federationAddresses[w.id]
                            ? `${federationAddresses[w.id]} · ${t("balance.activeVerified")}`
                            : t("balance.activeVerified")}
                        </span>
                        {" "}↗
                      </a>
                    ) : (
      ...
      <div className="bg-card border border-border rounded-xl shadow-xs overflow-hidden">
        {wallets.length === 0 && !loading ? (
          <div className="py-10 text-center text-sm text-muted-foreground">{t("balance.noWallets")}</div>
        ) : (
          <table className="w-full border-collapse">
            <thead>
              <tr style={{ background: "hsl(var(--theo-cream))" }}>
                {[t("balance.table.account"), t("balance.table.accountId"), t("balance.table.usdcAvailable"), t("balance.table.htgcAvailable"), t("balance.table.yieldBalance")].map((h) => (
        ...
        >
          ...
          >
            ...
            <div className="rounded-xl mb-5" style={{ background: "#F0FDF4", border: "1px solid #BBF7D0", padding: "14px 16px" }}>
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center gap-1.5">
                  ...
                  <span style={{ fontSize: 11, color: "#15803D" }}>{t("balance.paidToYou")}</span>
                </div>
            ...
            <div className="flex items-start gap-2 mb-5" style={{ fontSize: 12, color: "hsl(var(--theo-mid))" }}>
              ...
              <span>{t("balance.riskNotice")}</span>
            </div>
            ...
            <div className="flex gap-2">
              ...
              >
                {sweeping
                  ? <><Loader2 size={13} className="animate-spin" /> {t("balance.sweeping")}</>
          ...
          >
              ...
              >
                {wallets.map((w) => (
                  <option key={w.id} value={w.id}>
                    {w.label ?? "Wallet"} · {fmt(moveAssetBalances[w.id] ?? 0)} {moveAssetLabel} {t("balance.move.available")}
                  </option>
                ))}
              </select>
            </label>
            ...
            <label className="block mb-3">
              ...
              >
                {wallets.filter((w) => w.id !== moveSourceId).map((w) => (
                  <option key={w.id} value={w.id}>
                    {w.label ?? "Wallet"} · {fmt(moveAssetBalances[w.id] ?? 0)} {moveAssetLabel} {t("balance.move.balance")}
                  </option>
                ))}
              </select>
            </label>
            ...
            <label className="block mb-3">
              <div className="flex items-center justify-between mb-1.5">
                ...
                >
                  {t("balance.move.max").replace("{amount}", fmtMoveAmount(moveSourceBalance))}
                </button>
              </div>
            ...
            <label className="block mb-4">
              ...
              <input
                ...
                placeholder={t("balance.move.memoHint")}
                disabled={moving}
                className="w-full bg-card border border-border rounded-lg"
                style={{ padding: "10px 12px", fontSize: 13, fontFamily: "inherit", color: "hsl(var(--theo-ink))" }}
              />
            ...
            <div className="flex items-start gap-2 mb-4" style={{ fontSize: 11, color: "hsl(var(--theo-mid))" }}>
              ...
              <span>{t("balance.moveNotice")}</span>
            </div>
            ...
            <div className="flex gap-2 justify-end">
              ...
              >
                {moving
                  ? <><Loader2 size={13} className="animate-spin" /> {t("balance.moving")}</>
                  : <><ArrowLeftRight size={13} /> {moveAmountNum > 0 ? t("balance.move.confirm").replace("{amount}", fmtMoveAmount(moveAmountNum)) : t("balance.move.confirmEmpty")}</>}
              </button>
            </div>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            <div className="flex gap-2">
              ...
              >
                {blendWithdrawing
                  ? <><Loader2 size={13} className="animate-spin" /> {t("balance.withdrawing")}</>
...
function LedgerRow({
  w, idx, balance, htgcBalance, blendPosition, canViewBalances,
}: {
  w: Wallet; idx: number; balance: number; htgcBalance: number;
  blendPosition: BlendPosition | null; canViewBalances: boolean;
}) {
  const t = useT();
  ...
  return (
    <tr className="border-b border-border last:border-0 hover:bg-muted/40 transition-colors">
      <td className="px-5 py-3" style={{ fontSize: 13, fontWeight: 600 }}>{w.label ?? `${t("balance.table.account")} ${idx + 1}`}</td>
      <td className="px-5 py-3" style={{ fontFamily: "monospace", fontSize: 12 }}>
        {can("balance_view_keys") ? (
          show ? (
            ...
            >
              {t("balance.table.viewId")}
            </button>
          )
        ) : (
          <span style={{ color: "hsl(var(--theo-mid))", fontSize: 12 }}>—</span>
        )}
      </td>
      <td className="px-5 py-3">
        {canViewBalances ? (
          <div style={{ fontSize: 13, fontWeight: 700 }}>
            ${balance.toLocaleString(NUMBER_LOCALE, { minimumFractionDigits: 2, maximumFractionDigits: 2 })} USDC
          </div>
        ) : (
      ...
      <td className="px-5 py-3">
        {!canViewBalances ? (
          ...
          <div style={{ fontSize: 13, fontWeight: 700, color: "hsl(var(--theo-blue))" }}>
            {Math.round(htgcBalance).toLocaleString(NUMBER_LOCALE)}{" "}
        ...
        {!canViewBalances ? (
          ...
          <div>
            ...
            <div style={{ fontSize: 11, color: "#15803D", opacity: 0.8 }}>
              +${fmt(blendPosition.deposited * DEFAULT_NET_APY / 365)}/{t("balance.day")}
            </div>
          </div>
        ) : (
          <span style={{ fontSize: 12, color: "hsl(var(--theo-mid))" }}>—</span>
