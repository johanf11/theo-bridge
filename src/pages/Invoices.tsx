...
import {
  ...
  CheckCircle2, Clock, AlertTriangle, FileText, Loader2, X,
} from "lucide-react";
...
const STATUS_CONFIG: Record<InvoiceStatus, { label: string; bg: string; color: string; icon: React.ReactNode }> = {
  DRAFT:   { label: "Draft",    bg: "hsl(var(--theo-cream))",    color: "hsl(var(--theo-mid))",  icon: <FileText size={10} /> },
  SENT:    { label: "Sent",     bg: "hsl(var(--theo-blue-soft))", color: "hsl(var(--theo-blue))", icon: <Clock size={10} /> },
  PAID:    { label: "Paid",     bg: "#EFFBF3",                    color: "#1A7F37",               icon: <CheckCircle2 size={10} /> },
  OVERDUE: { label: "Overdue",  bg: "#FEE2E2",                    color: "#B91C1C",               icon: <AlertTriangle size={10} /> },
};
    ...
    <AppLayout>
      ...
      <div className="mb-1">
        ...
        <div style={{ fontSize: 13, color: "hsl(var(--theo-mid))", marginTop: 2 }}>
          Request USDC or HTG-C payments directly to your Theo wallet.
        </div>
      </div>
      ...
      <div className="grid gap-4" style={{ gridTemplateColumns: "3fr 2fr", alignItems: "start" }}>
        ...
        <div className="bg-card border border-border rounded-xl shadow-xs" style={{ padding: "18px 20px 16px" }}>
          <div className="font-bold mb-4" style={{ fontSize: 13, color: "hsl(var(--theo-blue))" }}>New invoice</div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="grid gap-3 mb-3" style={{ gridTemplateColumns: "1fr 1fr" }}>
              <div>
                <label style={labelStyle}>Client name <span style={{ color: "#C00" }}>*</span></label>
                <input
                  style={inputStyle}
                  placeholder="Acra Group"
                  value={clientName}
              ...
              <div>
                <label style={labelStyle}>Client email <span style={{ color: "hsl(var(--theo-mid))", fontWeight: 500, textTransform: "none", letterSpacing: 0 }}>(optional)</span></label>
                <input
                  style={inputStyle}
                  type="email"
            ...
            <div style={{ marginBottom: 12 }}>
              ...
              >
                {["Description", "Qty", "Unit price", "Total", ""].map((h) => (
                  <span key={h} style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.08em", color: "hsl(var(--theo-mid))" }}>
                    {h}
                  </span>
                ))}
              </div>
            ...
            {/* Discount */}
            <div style={{ marginBottom: 12 }}>
              <label style={labelStyle}>Discount <span style={{ color: "hsl(var(--theo-mid))", fontWeight: 500, textTransform: "none", letterSpacing: 0 }}>(optional)</span></label>
              <div style={{ display: "flex", gap: 8, alignItems: "center" }}>
                ...
                <div style={{ display: "flex", borderRadius: 8, border: "1.5px solid hsl(var(--theo-light))", overflow: "hidden", flexShrink: 0, height: 36 }}>
                  {([null, "flat", "percent"] as (DiscountType | null)[]).map((t) => (
                    <button
                      key={String(t)}
                      type="button"
                      onClick={() => { setDiscountType(t); if (!t) setDiscountValue(""); }}
                      style={{
                        padding: "0 10px", fontSize: 11, fontWeight: 700, border: "none",
                        cursor: "pointer", fontFamily: "inherit", transition: "all 120ms",
                        background: discountType === t ? "hsl(var(--theo-blue))" : "#fff",
                        color: discountType === t ? "#fff" : "hsl(var(--theo-mid))",
                      }}
                    >
                      {t === null ? "None" : t === "flat" ? "Flat" : "%"}
                    </button>
                  ))}
                </div>
                {discountType && (
            ...
            <div style={{ borderRadius: 10, background: "hsl(var(--theo-cream))", border: "1px solid hsl(var(--theo-light))", padding: "12px 14px", marginBottom: 14 }}>
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 6 }}>
                <span style={{ color: "hsl(var(--theo-mid))" }}>Subtotal</span>
                <span style={{ fontWeight: 600, color: "hsl(var(--theo-blue))" }}>{fmt(subtotal)} {currency}</span>
              </div>
              {discount > 0 && (
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, marginBottom: 6 }}>
                  <span style={{ color: "hsl(var(--theo-mid))" }}>Discount</span>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", fontSize: 15, fontWeight: 800, paddingTop: 8, borderTop: "1px solid hsl(var(--theo-light))" }}>
                <span style={{ color: "hsl(var(--theo-blue))" }}>Total</span>
            ...
            <div className="grid gap-3 mb-3" style={{ gridTemplateColumns: "1fr 1fr" }}>
              <div>
                <label style={labelStyle}>Payment wallet</label>
                {wallets.length === 0 ? (
                  <div style={{ fontSize: 13, color: "hsl(var(--theo-mid))" }}>No wallets — add one on Balance page.</div>
                ) : (
                  <select
                    style={{
                      ...inputStyle,
              ...
              <div>
                <label style={labelStyle}>Due date <span style={{ color: "hsl(var(--theo-mid))", fontWeight: 500, textTransform: "none", letterSpacing: 0 }}>(optional)</span></label>
                <input
                  style={inputStyle}
                  type="date"
                  value={dueDate}
            ...
            <div style={{ marginBottom: 14 }}>
              <label style={labelStyle}>Note to client <span style={{ color: "hsl(var(--theo-mid))", fontWeight: 500, textTransform: "none", letterSpacing: 0 }}>(optional)</span></label>
              <textarea
                style={{ ...inputStyle, resize: "vertical", minHeight: 64, lineHeight: 1.5 }}
                placeholder="Payment terms, bank details, project reference…"
                maxLength={200}
                value={note}
                  ...
                  <div key={inv.id} style={{ borderBottom: i < invoices.length - 1 ? "1px solid hsl(var(--theo-light))" : "none" }}>
                    ...
                    {isOpen && (
                      <div style={{ paddingBottom: 14, paddingTop: 2 }}>
                        ...
                        {/* Payment link notice */}
                        <div style={{ padding: "8px 10px", borderRadius: 7, background: "hsl(var(--theo-blue-soft))", border: "1px solid hsl(var(--theo-light))", marginBottom: 10, fontSize: 11, color: "hsl(var(--theo-blue))", lineHeight: 1.5 }}>
                          🔗 On-chain payment detection coming soon — the payment link below will open a hosted checkout page.
                        </div>
                        ...
                        <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                          ...
                          >
                            ...
                            {copiedId === inv.id ? "Copied!" : "Copy payment link"}
                          </button>
