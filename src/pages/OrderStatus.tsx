...
function getSteps(kind: string | null | undefined, reference: string | null | undefined, t: (key: TKey) => string) {
  if (kind === "htgc_mint") {
    return [
      { key: "QUOTED", label: t("order.step.depositRef"), sub: t("order.step.orderCreated") },
      { key: "FUNDED", label: t("order.step.paymentReceived"), sub: t("order.step.spihConfirmed") },
      { key: "RELEASING", label: t("order.step.mintingHtgc"), sub: t("order.step.stellarBroadcast") },
      { key: "COMPLETED", label: t("order.steps.complete"), sub: t("order.step.htgcInWallet") },
    ];
  }
  if (kind === "htgc_withdrawal" || (reference ?? "").startsWith("THEO-W-")) {
    return [
      { key: "QUOTED", label: t("order.step.redemptionRequested"), sub: t("order.step.orderCreated") },
      { key: "RELEASING", label: t("order.step.processingRedemption"), sub: t("order.step.stellarBroadcast") },
      { key: "COMPLETED", label: t("order.steps.complete"), sub: t("order.step.htgReleased") },
    ];
  }
  return [
    { key: "QUOTED", label: t("order.step.quoteLocked"), sub: t("order.step.rateConfirmed") },
    { key: "FUNDED", label: t("order.step.paymentReceived"), sub: t("order.step.spihConfirmed") },
    { key: "RELEASING", label: t("order.step.releasingUsdc"), sub: t("order.step.theoBroadcast") },
    { key: "COMPLETED", label: t("order.steps.complete"), sub: t("order.step.completeUsdc") },
  ];
}
...
export default function OrderStatus() {
  ...
  const [walletStellarAddress, setWalletStellarAddress] = useState<string | null>(null);
  ...
  // Fetch wallet label + stellar address — by wallet_id, or fall back to
  // destination_stellar_address (legacy swap orders may not persist wallet_id).
  useEffect(() => {
    if (!order) return;
    const fetchByWalletId = async () => {
      if (!order.wallet_id) return null;
      const { data } = await supabase
        .from("wallets")
        .select("label, stellar_address")
        .eq("id", order.wallet_id)
        .maybeSingle();
      return data;
    };
    const fetchByDestAddress = async () => {
      const addr = order.destination_stellar_address ?? order.destination_wallet_address;
      if (!addr) return null;
      const { data } = await supabase
        .from("wallets")
        .select("label, stellar_address")
        .eq("stellar_address", addr)
        .maybeSingle();
      return data;
    };
    (async () => {
      const byId = await fetchByWalletId();
      const wallet = byId ?? await fetchByDestAddress();
      setWalletLabel(wallet?.label ?? null);
  ...
  if (!order) {
    return <AppLayout><div className="text-muted-foreground">{t("order.loading")}</div></AppLayout>;
  }

  const STEPS = getSteps(order.order_kind, order.reference_number, t);
  const idx = stepIndex(order.status, STEPS);
  const isTerminalFail = order.status === "FAILED" || order.status === "EXPIRED";
  const isWithdrawal =
    order.order_kind === "htgc_withdrawal" ||
    (order.reference_number ?? "").startsWith("THEO-W-");
  const htgcFmt = (n: number) =>
    ...
    <AppLayout>
        ...
        <div className="flex items-start justify-between gap-4 mt-3">
          <div>
            <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-muted-foreground mb-2">
              {order.order_kind === "htgc_mint"
                ? t("order.depositOrder")
                : isWithdrawal
                  ? t("order.redemptionOrder")
                  : t("order.conversionOrder")}
            </div>
            <h1 className="text-3xl md:text-4xl font-extrabold text-theo-blue tracking-tight">
              {t("order.orderLabel")} <span className="ml-2">{order.reference_number}</span>
            </h1>
          ...
          {order.status === "QUOTED" && (
            <div className="inline-flex items-center gap-1.5 rounded-full bg-theo-gold-soft px-3 py-1.5 text-sm font-semibold text-theo-blue">
              <Hourglass className="h-3.5 w-3.5" /> {t("order.awaitingPayment")}
            </div>
          )}
          {(order.status === "FUNDED" || order.status === "RELEASING") && (
            <div className="inline-flex items-center gap-1.5 rounded-full bg-theo-cyan-soft px-3 py-1.5 text-sm font-semibold text-theo-cyan">
              <Loader2 className="h-3.5 w-3.5 animate-spin" /> {t("order.processing")}
            </div>
          )}
          {order.status === "COMPLETED" && (
            <div className="inline-flex items-center gap-1.5 rounded-full bg-success/10 px-3 py-1.5 text-sm font-semibold text-success border border-success/30">
              <CheckCircle2 className="h-3.5 w-3.5" /> {t("order.complete")}
            </div>
          )}
          {isTerminalFail && (
            <div className="inline-flex items-center gap-1.5 rounded-full bg-destructive/15 px-3 py-1.5 text-sm font-semibold text-destructive">
              {order.status === "EXPIRED" ? t("order.expired") : t("order.failed")}
            </div>
          )}
        </div>
      </div>
      ...
      {isWithdrawal ? (
        <div className="grid md:grid-cols-2 gap-4 mb-6">
          <div className="rounded-2xl bg-theo-gold p-5">
            <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-theo-blue/80">
              {t("order.htgcRedeemed")}
            </div>
            <div className="text-3xl font-extrabold text-theo-blue mt-2 tracking-tight">
              {htgcFmt(Number(order.htg_amount))} HTG-C
            </div>
          </div>
          <div className="rounded-2xl bg-card border p-5">
            <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-muted-foreground">{t("order.htgPaidOut")}</div>
            <div className="text-3xl font-extrabold text-theo-blue mt-2 tracking-tight">
        ...
        <div className="grid md:grid-cols-3 gap-4 mb-6">
          ...
          <div className="rounded-2xl bg-card border p-5">
            <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-muted-foreground">{t("order.htgDue")}</div>
            <div className="text-3xl font-extrabold text-theo-blue mt-2 tracking-tight">
              {fmtHTG(Number(order.htg_amount))}
            </div>
          </div>
          <div className="rounded-2xl bg-card border p-5">
            <div className="text-[11px] font-bold uppercase tracking-[0.18em] text-muted-foreground">{t("order.rate")}</div>
            <div className="text-3xl font-extrabold text-theo-blue mt-2 tracking-tight">
        ...
        <div className="rounded-2xl border bg-theo-blue-soft/60 mb-6 overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-border/50">
            <div className="flex items-center gap-2 text-lg font-bold text-theo-blue">
              <CreditCard className="h-4 w-4" /> {t("order.payViaSpih")}
            </div>
            <div className={cn(
              ...
              <span className="text-xs font-normal text-muted-foreground ml-1">{t("order.remaining")}</span>
            </div>
          </div>
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex items-start gap-3 rounded-xl border border-theo-gold/40 bg-theo-gold-soft/60 p-4">
              ...
              <p className="text-sm text-theo-blue">
                <span className="font-semibold">{t("order.memoWarning")}</span>{" "}
                <span className="text-muted-foreground">{t("order.memoWarningSub")}</span>
              </p>
            </div>
          </div>
        ...
        <div className="rounded-2xl mb-6 p-5 flex items-center gap-4" style={{ background: "hsl(var(--theo-cyan-soft))", border: "1.5px solid hsl(var(--theo-cyan) / 0.35)" }}>
          ...
          <div>
            <div className="font-bold text-base" style={{ color: "hsl(var(--theo-blue))", letterSpacing: "-0.01em" }}>
              {order.status === "FUNDED" ? t("common.status.paymentReceived") : order.order_kind === "htgc_mint" ? t("order.minting") : t("common.status.releasing")}
            </div>
            <div className="text-sm mt-0.5" style={{ color: "hsl(var(--theo-mid))" }}>
              {order.status === "FUNDED"
                ? t("order.releasing.queued")
                : t("order.releasing.confirming")}
            </div>
          </div>
        </div>
      )}
          ...
          <div className="rounded-2xl mb-4 overflow-hidden" style={{ border: "1.5px solid hsl(var(--theo-blue))" }}>
            ...
            <div className="flex items-center gap-5 px-6 py-5" style={{ background: "hsl(var(--theo-blue))" }}>
              ...
              <div className="flex-1 min-w-0">
                <div className="text-[10px] font-bold uppercase tracking-[0.18em] mb-1" style={{ color: "hsl(var(--theo-gold))" }}>
                  {order.order_kind === "htgc_mint"
                    ? t("order.depositComplete")
                    : isWithdrawal
                      ? t("order.redemptionComplete")
                      : t("order.conversionComplete")}
                </div>
                <div className="font-extrabold text-2xl tracking-tight" style={{ color: "#fff", letterSpacing: "-0.02em" }}>
                  {order.order_kind === "htgc_mint"
                    ? t("order.htgcDelivered").replace("{amount}", htgcFmt(Number(order.htg_amount)))
                    : isWithdrawal
                      ? t("order.htgcRedeemedAmount").replace("{amount}", htgcFmt(Number(order.htg_amount)))
                      : t("order.usdcDelivered").replace("{amount}", fmtUSDC(Number(order.usdc_amount)))}
                </div>
              </div>
              ...
              <div className="flex items-center gap-2 shrink-0">
                <button
                  onClick={() => generateReceipt({
                    kind: (isWithdrawal ? "withdraw" : (order.order_kind ?? "conversion")) as Parameters<typeof generateReceipt>[0]["kind"],
                    ...
                    sourceWalletAddress: walletStellarAddress,
                    destinationAddress: (() => {
                      const explicit = order.destination_stellar_address ?? order.destination_wallet_address;
                      if (explicit) return explicit;
                      const k = order.order_kind;
                      if (k === "usdc_conversion" || k === "htgc_usdc_swap" || k === "htgc_mint") return walletStellarAddress ?? undefined;
                      return undefined;
                    })(),
                    destinationWalletLabel: (() => {
                      const explicit = order.destination_stellar_address ?? order.destination_wallet_address;
                      if (explicit) return walletLabel ?? undefined;
                      const k = order.order_kind;
                      if (k === "usdc_conversion" || k === "htgc_usdc_swap" || k === "htgc_mint") return walletLabel ?? undefined;
                      return undefined;
                    })(),
                  })}
                  style={{
            ...
            >
              ...
              <span style={{ fontSize: 12, color: "hsl(var(--theo-mid))" }}>
                {isWithdrawal ? t("order.redeemedFrom") : t("order.settledTo")}{" "}
                <span style={{ fontWeight: 700, color: "hsl(var(--theo-ink))" }}>{settledTo}</span>
                {" · "}{t("order.stellarNetwork")}
              </span>
            </div>
          </div>
              ...
              <div className="px-5 py-4 space-y-3">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    ...
                    >
                      {t("order.viewTx")} <ExternalLink className="h-3 w-3" />
                    </a>
                  </div>
                </div>
