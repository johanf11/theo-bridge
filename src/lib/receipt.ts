...
export type ReceiptData = {
  ...
  memo?: string | null;
  walletLabel?: string;
  /** Stellar address of the source/initiating wallet — shown in Settlement section. */
...
function _buildPdf(data: ReceiptData): void {
  ...
  function drawStatus(status?: string) {
    ...
    // Green = settled/positive; amber = in progress; red = only genuine failures.
    const GREEN_STATES  = ["COMPLETED", "EARNED"];
    const AMBER_STATES  = ["PENDING", "PROCESSING", "FUNDED", "RELEASING", "ACCRUING", "EARNING"];
    const tone = GREEN_STATES.includes(s) ? "completed"
               : AMBER_STATES.includes(s) ? "pending"
               : "failed";
  ...
  if (kind === "conversion") {
    ...
    if (data.sourceWalletAddress) {
      const addr = data.sourceWalletAddress;
      const truncated = addr.slice(0, 6) + "…" + addr.slice(-6);
      const walletDisplay = data.walletLabel ? `${data.walletLabel} · ${truncated}` : truncated;
      drawRow("Source Wallet", walletDisplay);
    }
    if (data.stellarTxHash) {
      drawRow("On-chain", "CONFIRMED", { status: "completed" });
      drawTxHash(data.stellarTxHash);
