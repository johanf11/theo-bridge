...
Deno.serve(async (req) => {
  ...
  if (order.status !== "QUOTED" && order.status !== "FUNDED") {
    return err(`quote not available for conversion (status=${order.status})`, "quote_already_used", 409);
  }
  ...
  if (order.order_kind !== "usdc_conversion") {
    return err("convert is only required for HTG-C sourced quotes", "invalid_request", 400);
  }
