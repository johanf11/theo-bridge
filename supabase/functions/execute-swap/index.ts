...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Enforce org-level permission (Viewer role cannot convert)
    const { checkOrgPermission } = await import("../_shared/resolve-customer.ts");
    const permErr = await checkOrgPermission(admin, user.id, "convert");
    ...
    if (completed) {
      if (direction === "htgc_to_usdc") {
        await safePostLedger(admin, "execute-swap", {
          ...
          entries: [
            // HTG side: customer burns HTG-C → reduce outstanding liability
            { code: "HTGC_ISSUED",              currency: "HTG",  debit:  htgAmount },
            { code: "FX_CLEARING_HTG",          currency: "HTG",  credit: htgAmount },
            // USDC side: USDC leaves distributor (gross = net + fee)
        ...
        // IMPORTANT: leg1 sends USDC to TREASURY_PUBLIC (see leg1Destination above),
        // so the book MUST debit TREASURY_USDC — debiting DISTRIBUTOR_USDC here would
        // create a chain-vs-book drift on the distributor reconciler.
        if (leg1Destination !== TREASURY_PUBLIC) {
          throw new Error(
            `usdc_to_htgc invariant violated: leg1Destination ${leg1Destination} != TREASURY_PUBLIC ${TREASURY_PUBLIC}`,
          );
        }
        await safePostLedger(admin, "execute-swap", {
          orderId:     order.id,
          ...
          sourceKey:   `swap:${order.id}`,
          entries: [
            // USDC side: USDC arrives at TREASURY (cold wallet), not the distributor
            { code: "TREASURY_USDC",            currency: "USDC", debit:  usdcGross                          },
            { code: "FX_CLEARING_USDC",         currency: "USDC", credit: usdcNet                            },
            { code: "FEE_REVENUE_USDC",         currency: "USDC", credit: feeUsdc                            },
            // HTG side: new HTG-C minted → liability increases
