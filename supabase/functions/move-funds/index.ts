...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // Enforce org-level permission (Viewer role cannot move funds)
    ...
    // User-supplied memo — reject if it exceeds the 28-byte Stellar limit
    // (byte-aware, consistent with send-payment; never silently truncate).
    const userMemo = (memo ?? "").toString().trim();
    if (userMemo) {
      if (new TextEncoder().encode(userMemo).length > 28) {
        return json({ error: "Text memo exceeds 28 bytes." }, 400);
      }
      txBuilder.addMemo(Memo.text(userMemo));
    } else {
      txBuilder.addMemo(Memo.text("theo-transfer"));
    }
