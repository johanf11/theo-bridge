...
/**
 * For org members, asserts that their role has a given permission enabled.
 * Org owners always pass (they are not in org_members).
 * Throws a Response-compatible error object if the check fails.
 *
 * @param admin      Service-role Supabase client
 * @param userId     auth.users.id of the caller
 * @param permission e.g. "convert" | "payout_send" | "accounts_manage"
 * @returns          Error message string if permission denied, null if allowed
 */
export async function checkOrgPermission(
  ...
  // Not an org member → is the org owner → always allowed
  ...
  if (!perm?.enabled) return `Your role does not have permission to perform this action (${permission})`;
  return null;
}
