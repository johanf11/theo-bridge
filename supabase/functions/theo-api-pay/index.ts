...
// Executes the previously quoted payment to the supplier's Stellar address.
// Reuses execute-swap (HTG-C source) or send-payment (USDC source) internally
// by going straight to the Stellar SDK with the distributor / wallet signer.
...
Deno.serve(async (req) => {
  ...
  let hash: string;
  try {
    // Ensure distributor has enough USDC; mint from issuer if short (testnet).
    ...
    const storedMemo = (order as { payout_memo?: string | null }).payout_memo ?? null;
    const storedMemoType = ((order as { payout_memo_type?: "text" | "id" | null }).payout_memo_type) ?? "text";
    let memo;
    if (storedMemo) {
  ...
  // If source was HTG-C, burn the customer's HTG-C against the distributor for accounting.
  // For now we just mark COMPLETED — the ledger will reflect via the existing order flow.
