-- ─────────────────────────────────────────────────────────────────────────────
-- Patch: insert missing ledger_entries for USDC mint transactions.
--
-- Root cause chain:
--   1. _ledger_validate_balance trigger referenced side/amount columns that
--      don't exist (ledger_entries uses debit/credit). Fixed by 20260523120000.
--   2. Deferred trigger rolled back every ledger_entries INSERT.
--   3. But ledger_transactions INSERT committed first (inside post_ledger_entries
--      which inserts tx header before entries — all in the same plpgsql function,
--      so both roll back on trigger failure). Actually no entries were orphaned
--      from safePostLedger. They were orphaned from a prior direct SQL run.
--   4. The 20260523000001 backfill used amount/side columns (wrong schema) and
--      skipped all rows via CONTINUE WHEN EXISTS (some tx rows already existed).
--
-- This migration handles all cases:
--   A. Transaction row exists but has zero entries → insert entries.
--   B. Transaction row is missing → create it and insert entries.
--
-- Uses debit/credit columns (the real schema in this DB).
-- Idempotent: checks entry count before inserting.
-- ─────────────────────────────────────────────────────────────────────────────

-- Step 1: also re-fix _ledger_validate_balance (idempotent, in case the
-- 20260523120000 migration hasn't been applied yet on this environment).
...
BEGIN
  ...
  LOOP
    ...
    -- Find or create the transaction header
    ...
    -- Only insert entries if none exist yet for this tx
...
-- Step 3: same fix for the other scheduled-tx kinds (onramp, swap, payout)
-- that may also have orphaned tx headers with no entries.
DO $$
DECLARE
  v_tx    record;
  v_entry_count int;
