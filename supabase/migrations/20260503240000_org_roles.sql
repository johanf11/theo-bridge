...
-- Helper: is current user the org owner?
...
begin
  ...
  -- Treasury Analyst: can convert + send, no keys, no account management
  insert into role_permissions (role_id, permission, enabled) values
...
-- ── Trigger: seed roles on new customer ──────────────────────────────────────
